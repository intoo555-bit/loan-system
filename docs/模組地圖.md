...
##### ⛔ 判斷「業務打的是概略名稱嗎」——要用原始輸入，不能用 alias 之後的值

`COMPANY_ALIAS` 會先把概略名換成預設產品（`21` → `21商品`、`貸10` → `貸救補`），
之後再判斷「這是不是概略名稱」就永遠不成立：

```python
company = COMPANY_ALIAS.get(company, company)   # 「21」→「21商品」
_co_norm = normalize_section(company)           # 「21商品」→「21」
if company == _co_norm:                         # 「21商品」==「21」→ 永遠 False ❌
```

2026-08-05 的實際災情（`update_amount`）：客戶在送 `21機車12萬`，A 群打「`21 核准 25萬`」，
上面那個條件永遠 False → 「用客戶實際在送那家」的防護形同虛設 →
`current_company` 被改成 `21商品`，但 `route_plan` 的 order 還是 `21機車12萬`，**資料自相矛盾**、日報顯示錯的方案。
...
| 業務打 | 是概略名？ | 該怎麼做 |
|---|---|---|
| `21` / `亞太` | ✅ | 換成客戶實際在送的那家（`21機車12萬`） |
| `21機車12萬` / `21商品` | ❌ | 保留原值 |
| `慢點付`（alias→零卡） | ❌ | 保留「慢點付」，不要換成零卡 |

擋這件事的測試：`test_flows.py` 第 7 項。
