...
## 2026/08/08 統計數字錯 + 英文外洩

### 核准漏算一半以上
`@AI 統計` 的核准數只掃 `status='ACTIVE'` 的客戶 → 核准後已撥款結案的
全部不算。實測本月：只算 ACTIVE 27 件、含已結案 62 件。
改為 `status!='DELETED'` 全算。

### 結案用 updated_at → 新增 closed_at
`updated_at` 是最後異動時間。黃俊仁 5/8 結案，8/6 救資料動了兩次，
就被算成 8 月結案、同時從 5 月消失。

- 新增 `customers.closed_at`
- `update_customer` 在「非結案 → 結案」時寫入（重啟後再結案會更新，那是新的申請週期）
- `migrate_db` 一次性回填：從 `case_logs` 抓最後一筆含「結案」的時間
  （實測 1549 筆回填 1542 筆，找不到的 7 筆 fallback `updated_at`）
- 統計改用 `COALESCE(NULLIF(closed_at,''), updated_at)`
- 效果：本月結案 67 → 65，黃俊仁回到 2026-05-08

### 英文外洩（使用者第二次抱怨）
| 位置 | 原本 | 改成 |
|---|---|---|
| 結案訊息 | `（route_plan 跟撥款記錄都清掉）` | `（送件順序和撥款紀錄都清掉）` |
| 已補訊息 | `（pending_docs 也清掉：…）` | `（缺件清單也清掉：…）` |
| 選案件失敗 | `（case_id=xxx）` | `（編號 xxx）` |
| 新增客戶警告 | `舊案狀態：CLOSED` | `舊案狀態：已結案` |

加 `STATUS_ZH` / `status_zh()` 統一轉換。
⛔ 第一次（08/03）只修了還原顯示那一處沒全域掃，才會有第二次。

測試：`test_flows.py` 第 41 項。

---

## 2026/08/10 換書籤／分頁圖示

### 技術重點：檔名 .ico 不夠，內容也要是真的 ICO
`static/favicon.png` 直接用 `/favicon.ico` 路由送出去 → 網頁分頁看得到圖，
但**書籤圖示不會更新**。改成 `im.save(..., format="ICO", sizes=[(16,16),(32,32),(48,48),(64,64)])`
產生真 ICO、路由回 `image/x-icon` 才生效。

### 其他這次踩到的
- **光有 `/favicon.ico` 路由不夠**：瀏覽器不一定會自己去要，要在頁面 `<head>` 明講。
  這個檔案有 42 個頁面各自寫 head，其中 20 個吃共用的 `PAGE_CSS`、其餘各自寫，
  且引號有 `"` 和 `'` 兩種 —— 插入 link 時要配合原本的引號，否則會把 f-string 本身弄壞。
- **圖案要用眼睛確認**：原圖是「板夾＋資料夾＋放大鏡＋房子＋勾選框」的組合圖，
  縮到書籤的 16px 完全糊掉。要實際產生 16px 放大圖來看，不能憑感覺說「應該可以」。
- **從原圖挖元素會有殘渣**：資料夾和放大鏡在原圖裡是疊在一起的，
  裁切、塗改、補線都留下痕跡（右框超出上緣的小尖角、左右殘留的小凸起）。
  最後改成用 PIL 直接畫（配色從原圖取樣），才乾淨。
- **補正方形要四邊都留白**：資料夾是扁的，只補上下會讓左右貼邊，縮小後看起來像被切掉。

### ⚠️ 過程本身的教訓
使用者回報四次「還是一樣」，我每次都判斷成他的瀏覽器快取。
真正的轉折是他截圖顯示「同一台電腦上另外兩個系統的書籤圖示都正常」——
有對照組就能立刻排除環境因素。見主檔「五條最貴的教訓」第 5 條。
