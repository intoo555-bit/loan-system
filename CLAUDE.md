...
| 改「公司簡稱怎麼對到客戶在送的方案」（核准／撥款／婉拒都會經過） | `docs/模組地圖.md` → 公司名比對 ⛔ 有一個 alias 順序陷阱，2026-08-05 害「21 核准」核到錯的公司 |
...
4. **報告「已完成」前，必須真的執行過工具並看到結果。**
   只在回覆裡寫出「要改成什麼」**不算完成** —— 2026-08-02 就發生過：
   我寫了「我改掉了，第 140 行改成…」，實際上一個字都沒動，
   是使用者說「那你改吧」才發現。
   還沒動手就明講「**以下是我打算改的，還沒動手**」，不要用完成式。

### ⛔ 動到客戶資料一律走 `update_customer`，不要直接下 SQL

`update_customer` 會寫 `case_logs` + 存改動前快照，所以查得到誰改的、也還原得回來。
直接 `cur.execute("UPDATE customers SET ...")` 就什麼都沒有。

已經因此出事兩次：

| 何時 | 哪裡直接下 SQL | 後果 |
|---|---|---|
| 2026-08-03 | `/report/update-progress` | 網頁改錯進度，查不到誰改的、沒有快照可還原（已修） |
| 2026-08-06 | `_dedupe_same_id_in_group` 的軟刪除 | 業務的案子 15:06 憑空消失，`case_logs` 一筆紀錄都沒有，最後是比對三份備份的 `updated_at` 才拼出時間軸 |

⚠️ 去重那處**目前仍是直接下 SQL**（只修了「不該刪」的邏輯，還沒補紀錄）。

### 使用者可以用這三句話戳破我最常犯的錯

| 什麼時候問 | 問這句 |
|---|---|
| 我說「我認為 / 應該是 / 看起來是」 | **「你實際跑過了嗎？」** |
| 我報告做完了 | **「這是已經做了，還是打算做？」** |
| 開工第一句 | **「memory 有幾條？」**（答不出 22 條 = 開錯資料夾，這次對話我什麼都不知道） |
