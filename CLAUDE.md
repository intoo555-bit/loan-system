...
### ⛔ 給業務看的訊息一律中文，不可出現欄位名或狀態代碼

業務看不懂 `route_plan`、`pending_docs`、`case_id`、`CLOSED`。
狀態一律用 `status_zh()` 轉；欄位名用中文寫（送件順序／缺件清單／編號）。

使用者為此抱怨過兩次。**第一次我只修了「還原顯示 JSON」那一處，沒有全域掃 ——
結果第二次又在別的地方冒出來**（結案訊息的「route_plan 跟撥款記錄都清掉」）。
→ 這類問題**一定要 grep 整個檔案**，修一處不算修完。

### ⛔ 統計的兩個坑（2026-08-08 實測）

**一、算業績不可以只掃 `status='ACTIVE'`**
核准→撥款→結案的客戶（跑完整流程、最成功的那些）會全部被漏掉。
實測：只算 ACTIVE 是 27 件，含已結案的實際 62 件，**漏了一半以上**。

**二、時間統計不可以用 `updated_at`**
那是「最後被動到的時間」，不是「事情發生的時間」。舊案這個月被碰一下
（改資料、救資料）就會被算成本月，**而且會從原本那個月的統計消失** ——
歷史數字被之後的操作侵蝕，今天看五月的數字跟三個月前不一樣。

→ 用專屬欄位：結案看 `closed_at`、核准看 `approved_at`、撥款看 `disbursement_date`。
   `closed_at` 由 `update_customer` 在「非結案→結案」時寫入，
   舊資料由 `migrate_db` 從 `case_logs` 回填（1549 筆補到 1542 筆）。
...
5. **使用者說「還是不行」時，先懷疑自己的實作，不要歸咎於環境、快取或他的操作。**
   2026-08-10 換書籤圖示：檔案叫 `favicon.ico` 但內容其實是 PNG，
   網頁分頁顯示得出來、**書籤圖示卻不更新**。
   使用者連續回報四次「還是一樣」，我每次都回「是你的瀏覽器快取，刪掉書籤重加」，
   還教他去刪瀏覽器的 Favicons 資料庫 —— **問題從頭到尾在我這邊**。
   最後是他截圖給我看：同一台電腦、同一個瀏覽器，另外兩個系統的書籤圖示都正常，
   只有這個不變。
...
1. **原本要解的問題真的解掉了嗎？** 對照**當初的需求**，不是對照我做了什麼。
2. **驗證了什麼、怎麼驗的、還有什麼沒驗？** 沒驗的**明說**。
   單元測試過／真實資料跑過／上線驗過 —— 這三件事完全不同。
3. **這次改動新引入什麼風險？** 預設值安全嗎、有沒有破壞既有行為、
   **失敗時會不會是安靜的**（log 看起來成功、實際沒做到）？
4. **有沒有留下不一致？** 文件、註解、其他呼叫端、另一台機器、排程。
5. **誰在用這個東西？他們知道改了嗎？** 使用方不是我就要主動通知，
   內容寫「**對方視角的用法變化**」，不是「我做了什麼」。

**第二段：還能怎麼優化** —— 最多 2～3 條，每條具體可執行並標「值不值得現在做」。
