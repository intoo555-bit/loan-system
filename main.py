...
PLAN_INFO = {
    ...
    "貸救補10": ("貸救補", "10萬/24期"), "貸就補10": ("貸救補", "10萬/24期"),
...
PLAN_ELIGIBILITY_RULES = [
    ...
    # ===== 預付資融系列 =====
    # 1 親 1 友 = 1 個二等親 + 1 個朋友、或 2 個二等親、其 1 名須知情
    {
        "company": "手機原融",
        "max_amount": 25,
        "priority": 72,
        "rules": [
            {"type": "simple", "label": "年齡 20~59", "field": "age", "op": "between", "value": [20, 59]},
            {"type": "simple", "label": "中華民國身分證", "field": "id_no", "op": "tw_id", "value": True},
            {"type": "simple", "label": "勞保/軍保/公保滿 1 年",
             "field": "eval_labor_ins", "op": "in", "value": ["公司保", "軍保", "公保"],
             "manual_check": "需確認滿 1 年"},
            {"type": "manual", "label": "近半年薪轉或存摺明細",
             "hint": "須附近 6 個月薪轉紀錄或存摺明細"},
            {"type": "simple", "label": "至少 1 位聯絡人知情（1 親 1 友其 1 須知情）", "op": "any_contact_known"},
            {"type": "simple", "label": "🚫 警示戶不做（自動偵測）", "op": "no_warning_account"},
        ],
        "required_docs": ["身分證正反", "第二證件（健保卡/駕照）", "存摺封面",
                          "近半年薪轉/存摺明細", "人與手機合照（時間相機）", "手機型號", "imei"],
    },
    {
        "company": "機車原融",
        "max_amount": 20,
        ...
        "rules": [
            {"type": "simple", "label": "年齡 20~59", "field": "age", "op": "between", "value": [20, 59]},
            {"type": "simple", "label": "中華民國身分證", "field": "id_no", "op": "tw_id", "value": True},
            {"type": "simple", "label": "車齡 ≤ 10 年", "field": "vehicle_year", "op": "year_age_le", "value": 10},
            {"type": "simple", "label": "原融（機車無貸款）", "op": "not_has_dynbao",
             "manual_check": "原融 = 機車本身無貸款、有貸款請看汽機車二胎方案"},
            {"type": "simple", "label": "勞保/軍保/公保滿 1 年",
             "field": "eval_labor_ins", "op": "in", "value": ["公司保", "軍保", "公保"],
             "manual_check": "需確認滿 1 年"},
            {"type": "manual", "label": "近半年薪轉或存摺明細"},
        ...
        "required_docs": ["身分證正反", "第二證件（健保卡/駕照）", "存摺封面",
                          "近半年薪轉/存摺明細", "人與機車合照（時間相機）", "機車行照", "牌照登記書"],
    },
    {
        "company": "汽機車二胎",
        "max_amount": 20,
        ...
        "rules": [
            {"type": "simple", "label": "年齡 20~59", "field": "age", "op": "between", "value": [20, 59]},
            {"type": "simple", "label": "中華民國身分證", "field": "id_no", "op": "tw_id", "value": True},
            {"type": "simple", "label": "車齡 ≤ 5 年", "field": "vehicle_year", "op": "year_age_le", "value": 5},
            {"type": "simple", "label": "二胎（原本有貸款也可送、不用代償還清）", "op": "has_dynbao",
             "manual_check": "二胎 = 原車有貸款再貸一次、不用先還清"},
            {"type": "simple", "label": "勞保/軍保/公保滿 1 年",
             "field": "eval_labor_ins", "op": "in", "value": ["公司保", "軍保", "公保"],
             "manual_check": "需確認滿 1 年"},
            {"type": "manual", "label": "近半年薪轉或存摺明細"},
            {"type": "manual", "label": "近半年本車前貸繳款收據 或 完整繳息聯徵報告（要繳款正常）"},
            {"type": "simple", "label": "至少 1 位聯絡人知情（1 親 1 友其 1 須知情）", "op": "any_contact_known"},
            {"type": "simple", "label": "🚫 警示戶不做（自動偵測）", "op": "no_warning_account"},
        ],
        "required_docs": ["身分證正反", "第二證件（健保卡/駕照）", "存摺封面",
                          "近半年薪轉/存摺明細", "人與機車合照（時間相機）", "機車行照",
                          "近半年前貸繳款收據 或 聯徵報告"],
    },
    {
        "company": "手機無卡分期",
    ...
    # ===== 銀行 =====
    {
        ...
        "rules": [
            ...
            {"type": "manual", "label": "💡 排序：條件符合就排（多家銀行、各家規則略不同）"},
            {"type": "simple", "label": "🚫 警示戶不做（自動偵測）", "op": "no_warning_account"},
        ],
        "required_docs": ["身分證正反", "第二證件（健保卡/駕照）", "存摺封面"],
    },
    # ===== 鄉民 =====
    {
        ...
        "max_amount": 12,
        "priority": 52,
        "always_eligible": True,  # 一律排進去、信用報告只看銀行不看私設
        "rules": [
            ...
            {"type": "manual", "label": "💡 排序：固定都排（融資商品、信用報告只看銀行不看動保私設）"},
            {"type": "simple", "label": "🚫 警示戶不做（自動偵測）", "op": "no_warning_account"},
        ],
        "required_docs": ["身分證正反", "第二證件（健保卡/駕照）", "存摺封面",
    ...
    # 共同特性：不看條件、無條件排給客戶申請、警示戶可送、現金撥二等親或朋友都可
    {
        ...
        "rules": [
            ...
            {"type": "manual", "label": "💡 排序：固定都排（每個客戶都會送）"},
        ],
        "required_docs": ["身分證正反", "第二證件（健保卡/駕照）", "存摺封面"],
    },
    {
    ...
    },
    {
        ...
        "max_amount": 10,
        "priority": 20,
        "always_eligible": True,  # 一律排進去（勞保滿 3 月時優先）
        "rules": [
            {"type": "simple", "label": "中華民國身分證", "field": "id_no", "op": "tw_id", "value": True},
            {"type": "manual", "label": "💡 民間貸款、不看條件、無條件排給客戶申請"},
            {"type": "manual", "label": "💡 警示戶可送、通常對保當下直接拿現金"},
            {"type": "manual", "label": "💡 排序：勞保滿 3 個月排代書、不滿 3 月（或無勞保）改排當鋪"},
        ],
        "required_docs": ["身分證正反", "第二證件（健保卡/駕照）", "存摺封面"],
    },
    {
        ...
        "max_amount": 5,
        "priority": 18,
        "always_eligible": True,  # 一律排進去（勞保不滿 3 月時取代代書）
        "rules": [
            {"type": "simple", "label": "中華民國身分證", "field": "id_no", "op": "tw_id", "value": True},
            {"type": "manual", "label": "💡 民間貸款、不看條件、無條件排給客戶申請"},
            {"type": "manual", "label": "💡 警示戶可送、通常對保當下直接拿現金"},
            {"type": "manual", "label": "💡 排序：勞保不滿 3 月或無勞保 → 排當鋪（取代代書）"},
        ],
        "required_docs": ["身分證正反", "第二證件（健保卡/駕照）", "存摺封面"],
    },
    # ===== 貸救補（貸10）=====
        ...
        "rules": [
            ...
            {"type": "oneof", "label": "財務能力（擇一、5 選 1）", "options": [
                ...
                {"type": "simple", "label": "持有不動產（不可有私設）",
                 "field": "eval_property", "op": "contains", "value": "不動產",
                 "exclude_field": "eval_alert", "exclude_value": "有"},
                {"type": "simple", "label": "同業（21體系）有貸款繳滿 6 期+（自動偵測）", "op": "group21_paid_ge", "value": 6},
    ...
    # B 專案（100 萬）需符合 14 項中的 1 項（12/13/14 不適用）
    # C 專案（50 萬）沒有符合也可送
    # 項目共 14 項、絕大多數系統無法自動判別、列為 manual
    {
        "company": "21汽車A",
        "max_amount": 200,
        "priority": 88,
        "rules": [
            ...
            {"type": "manual", "label": "車輛持有滿半年 或 新車一手車主、親屬過戶件可送"},
            {"type": "manual", "label": "✅ 接受三種情境：原車（原融）、他行代償、借新還舊",
             "hint": "前貸不限公司、原車無貸款也可、同公司再貸也可"},
            {"type": "manual", "label": "需符合下列 14 項中『至少 2 項』（A 專案）",
             "hint": "1.現職滿 9 月  2.不動產（RE 不可持分）  3.公司設立 9 月+  4.申貸成數 ≤ 70%  "
                     "5.廿一既有客戶繳 6 期+無逾期  6.近 15 天聯徵正常（不可小白）  "
                     "7.醫師/會計師/律師/建築師  8.保人三等親+正職  9.軍公教/國營/上市櫃  "
                     "10.甲乙丙丁技術士證照  11.儲蓄/壽/醫療險滿 3 月  "
                     "12.車齡 7 年內  13.持車滿 2 年無貸 + 車齡 10 年內  "
                     "14.持有 TOYOTA/NISSAN/中華/福特/HONDA/BMW/BENZ/VOLVO/LEXUS/AUDI/HYUNDAI 滿 6 月+車齡 5 年內"},
        ],
        "required_docs": ["身分證正反", "第二證件（健保卡/駕照）", "存摺封面",
                          "汽車行照", "天書 或 權威鑑價"],
    },
    {
        ...
        "rules": [
            ...
            {"type": "manual", "label": "✅ 接受三種情境：原車（原融）、他行代償、借新還舊",
             "hint": "前貸不限公司、原車無貸款也可、同公司再貸也可"},
            {"type": "manual", "label": "需符合下列 11 項中『至少 1 項』（B 專案、不含車輛條件 12/13/14）",
             "hint": "1.現職滿 9 月  2.不動產  3.公司設立 9 月+  4.申貸成數 ≤ 70%  "
                     "5.廿一既有客戶繳 6 期+無逾期  6.近 15 天聯徵正常  "
                     "7.醫師/會計師/律師/建築師  8.保人三等親+正職  9.軍公教/國營/上市櫃  "
                     "10.專業證照  11.儲蓄/壽/醫療險滿 3 月（B 專案不看車輛條件）"},
        ],
        "required_docs": ["身分證正反", "第二證件（健保卡/駕照）", "存摺封面",
                          "汽車行照", "天書 或 權威鑑價"],
    },
    {
        ...
        "rules": [
            ...
            {"type": "manual", "label": "✅ 接受三種情境：原車（原融）、他行代償、借新還舊",
             "hint": "前貸不限公司、原車無貸款也可、同公司再貸也可"},
            {"type": "manual", "label": "💡 C 專案：14 項都不符合也可送（基本款、最高 50 萬）"},
        ],
        "required_docs": ["身分證正反", "第二證件（健保卡/駕照）", "存摺封面",
                          "汽車行照", "天書 或 權威鑑價"],
    },
    # ===== 第一系列（EGO）=====
    {
        ...
        "rules": [
            {"type": "simple", "label": "年齡 20~65", "field": "age", "op": "between", "value": [20, 65]},
            {"type": "simple", "label": "中華民國身分證", "field": "id_no", "op": "tw_id", "value": True},
            {"type": "simple", "label": "客戶必須有汽車貸款（沒車貸不能送）", "op": "has_car_loan",
             "manual_check": "需確認該汽車貸款繳滿 1 年+遲繳不超過 15 天"},
            {"type": "simple", "label": "勞保/軍保/公保滿半年（須一週內）",
             "field": "eval_labor_ins", "op": "in", "value": ["公司保", "軍保", "公保"],
             "manual_check": "需確認滿半年 + 勞保須一週內"},
            {"type": "manual", "label": "汽車貸款繳款滿 1 年、遲繳不可超過 15 天",
             "hint": "看負債明細的設定日期 1 年以上、查繳款狀況"},
            {"type": "simple", "label": "🚫 警示戶不做（自動偵測）", "op": "no_warning_account"},
        ],
    ...
    },
    {
        ...
        "rules": [
            {"type": "simple", "label": "年齡 20~65", "field": "age", "op": "between", "value": [20, 65]},
            {"type": "simple", "label": "中華民國身分證", "field": "id_no", "op": "tw_id", "value": True},
            {"type": "simple", "label": "客戶必須有銀行信貸紀錄（沒銀行貸款不能送）", "op": "has_bank_loan",
             "manual_check": "需確認近一個月內有銀行信貸撥款"},
            {"type": "simple", "label": "勞保/軍保/公保滿半年（須一週內）",
             "field": "eval_labor_ins", "op": "in", "value": ["公司保", "軍保", "公保"],
             "manual_check": "需確認滿半年 + 勞保須一週內"},
            {"type": "simple", "label": "銀行信貸須是近一個月內撥款的（自動偵測 pa≤1）", "op": "recent_bank_loan"},
            {"type": "simple", "label": "🚫 警示戶不做（自動偵測）", "op": "no_warning_account"},
        ],
        "required_docs": ["身分證正反", "第二證件（健保卡/駕照）", "存摺封面", "勞保/軍保/公保（半年以上）",
    ...
    # ===== 興達系列 =====
    # 共同：年齡 20~55、最高 10 萬、最長 18 期
    # 承作區域：台灣本島 + 澎湖 + 金門（連江/馬祖不可）
    # 聯絡人：2 位、其 1 須為親屬、其 1 須知情
    {
        "company": "興達汽機車二胎",
        "max_amount": 10,
        "priority": 49,
        "rules": [
            {"type": "simple", "label": "年齡 20~55", "field": "age", "op": "between", "value": [20, 55]},
            {"type": "simple", "label": "中華民國身分證", "field": "id_no", "op": "tw_id", "value": True},
            {"type": "simple", "label": "現居須在台灣本島/澎湖/金門（連江/馬祖不可、戶籍可在外島、看現居）",
             "field": "live_city", "op": "not_contains", "value": ["連江", "馬祖"]},
            {"type": "simple", "label": "至少 2 位聯絡人、其中 1 位為親屬", "op": "any_contact_2nd_kin"},
            {"type": "simple", "label": "至少 1 位聯絡人知情", "op": "any_contact_known"},
            {"type": "manual", "label": "前案須繳滿 6 期(含)以上、不能遲繳超過 7 天",
             "hint": "看負債明細的前案繳款狀況"},
            {"type": "manual", "label": "需提供近半年完整繳息紀錄"},
            {"type": "manual", "label": "💡 最高金額 10 萬、最長期數 18 期"},
            {"type": "simple", "label": "🚫 警示戶不做（自動偵測）", "op": "no_warning_account"},
        ],
        "required_docs": ["身分證正反", "第二證件（健保卡/駕照）", "存摺封面", "汽/機車行照", "近半年完整繳息紀錄"],
    },
    {
        "company": "興達勞保貸",
        "max_amount": 10,
        ...
        "rules": [
            ...
            {"type": "simple", "label": "至少 2 位聯絡人、其中 1 位為親屬", "op": "any_contact_2nd_kin"},
            {"type": "simple", "label": "至少 1 位聯絡人知情", "op": "any_contact_known"},
            {"type": "simple", "label": "近半年薪轉 或 半年勞/軍/公保",
             "field": "eval_labor_ins", "op": "in", "value": ["公司保", "軍保", "公保"],
             "manual_check": "需確認勞/軍/公保滿半年、或薪轉滿半年"},
            {"type": "manual", "label": "名下所有融資/小額分期需附完整繳息+收據",
             "hint": "有動保的全部要附；沒動保但有融資（手機/商品貸款等）只要提供一項完整繳息即可"},
            {"type": "manual", "label": "名下貸款需繳滿 6 期以上、提供近半年完整繳息紀錄"},
            {"type": "manual", "label": "💡 最高金額 10 萬、最長期數 18 期"},
            {"type": "simple", "label": "🚫 警示戶不做（自動偵測）", "op": "no_warning_account"},
        ],
        "required_docs": ["身分證正反", "第二證件（健保卡/駕照）", "存摺封面",
                          "手機型號", "imei", "人與手機合照",
                          "近半年薪轉/勞保紀錄", "名下融資完整繳息+收據"],
    },
    # ===== 融易系列 =====
    # 共同：18~65 歲（>55 須補保人）、聯絡人 2 位其 1 須二等親
    # 承作區域：只做台灣本島、外島(澎湖/金門/連江/馬祖)皆不做
    # 警示戶不做
    # 基數表（試算用）：24期=489、36期=351、48期=283、60期=242
    {
        "company": "融易頭家貸",
        "max_amount": 100,
        "priority": 70,
        "rules": [
            {"type": "manual", "label": "年齡 18~65（超過 55 歲須補保人）",
             "hint": "硬性 18~65、55 以上要補保人"},
            {"type": "simple", "label": "中華民國身分證", "field": "id_no", "op": "tw_id", "value": True},
            {"type": "simple", "label": "現居須在台灣本島（澎湖/金門/連江/馬祖皆不做、戶籍在外島可、看現居）",
             "field": "live_city", "op": "not_contains", "value": ["澎湖", "金門", "連江", "馬祖"]},
            {"type": "simple", "label": "至少 2 位聯絡人、其中 1 位為二等親", "op": "any_contact_2nd_kin"},
            {"type": "manual", "label": "需有實體店面（流動攤販不承作）"},
            {"type": "manual", "label": "對保須在工作地"},
            {"type": "manual", "label": "進件需附店面照（招牌/門牌/外觀）+ 工作照 + 財產清單"},
            {"type": "manual", "label": "💡 額度（符 2 項條件）：機車→20萬/36期、汽車→50萬/36期、房屋→100萬/60期"},
            {"type": "manual", "label": "符兩項條件（7 選 2）",
             "hint": "1.攤商合約 2 年+（附合約書）  2.持有專業證照與工作相符  "
                     "3.近半年完整往存基數萬元以上  4.自有 RE 滿 1 年（無私設）  "
                     "5.二等親屬保(≤65)或正職保人  6.近 15 天內個人 JCIC（信用空白可）  "
                     "7.同業/銀行既有客戶繳滿 10 期+貸款期內無 15 天以上逾期"},
            {"type": "simple", "label": "🚫 警示戶不做（自動偵測）", "op": "no_warning_account"},
        ],
        "required_docs": ["身分證正反", "第二證件（健保卡/駕照）", "存摺封面",
                          "店面照（招牌/門牌/外觀）", "工作照", "財產清單"],
    },
    {
        "company": "融易BOSS微企貸",
        "max_amount": 100,
        ...
        "rules": [
            ...
            {"type": "simple", "label": "現居須在台灣本島（澎湖/金門/連江/馬祖皆不做、戶籍在外島可、看現居）",
             "field": "live_city", "op": "not_contains", "value": ["澎湖", "金門", "連江", "馬祖"]},
            {"type": "simple", "label": "至少 2 位聯絡人、其中 1 位為二等親", "op": "any_contact_2nd_kin"},
            {"type": "manual", "label": "需有營登"},
            {"type": "manual", "label": "對保須在工作地"},
            {"type": "manual", "label": "進件需附工作現勘照 + 統編（工作照申請人須入鏡 3-5 張）+ 財產清單"},
            {"type": "manual", "label": "💡 額度（符 2 項條件）：機車→20萬/36期、汽車→50萬/36期、房屋→100萬/60期"},
            {"type": "manual", "label": "符兩項條件（7 選 2）",
             "hint": "1.公司營登 2 年+（附營登或商業抄本）  2.持有信用卡 1 年+（附近一期完整帳單首頁）  "
                     "3.近半年完整往存基數萬元以上  4.自有 RE 滿 1 年（無私設）  "
                     "5.二等親屬保(≤65)或正職保人  6.近 15 天內個人 JCIC（信用空白可）  "
                     "7.同業/銀行既有客戶繳滿 10 期+貸款期內無 15 天以上逾期"},
            {"type": "simple", "label": "🚫 警示戶不做（自動偵測）", "op": "no_warning_account"},
        ...
        "required_docs": ["身分證正反", "第二證件（健保卡/駕照）", "存摺封面",
                          "營登或商業抄本", "工作現勘照（含申請人 3-5 張）", "統編", "財產清單"],
    },
    {
        "company": "融易機車資優專案",
        "max_amount": 40,
        ...
        "rules": [
            ...
            {"type": "simple", "label": "現居須在台灣本島（澎湖/金門/連江/馬祖皆不做、戶籍在外島可、看現居）",
             "field": "live_city", "op": "not_contains", "value": ["澎湖", "金門", "連江", "馬祖"]},
            {"type": "simple", "label": "至少 2 位聯絡人、其中 1 位為二等親", "op": "any_contact_2nd_kin"},
            {"type": "manual", "label": "⚠️ 會設定機車（私設）"},
            {"type": "manual", "label": "正職滿 2 年（負責人不認定）"},
            {"type": "manual", "label": "💡 額度：符 1 項→10萬/18期、2 項→20萬/24期、3 項→30萬/36期、4 項→40萬/36期"},
            {"type": "manual", "label": "符項條件（3 選 N）",
             "hint": "1.近 7 天內個人 JCIC  2.近 7 天內所得清單+財產清冊  "
                     "3.同業/銀行既有客戶繳滿 1 年+貸款期內無 10 天以上逾期（附完整繳息明細）"},
            {"type": "simple", "label": "🚫 警示戶不做（自動偵測）", "op": "no_warning_account"},
        ],
        "required_docs": ["身分證正反", "第二證件（健保卡/駕照）", "存摺封面", "機車行照"],
    },
    {
        ...
        "rules": [
            ...
            {"type": "simple", "label": "至少 2 位聯絡人、其中 1 位為二等親", "op": "any_contact_2nd_kin"},
            {"type": "manual", "label": "⚠️ 會設定機車（私設）"},
            {"type": "manual", "label": "申請人若無薪轉/勞保/軍保/公保 → 須附工作現勘照 + 往存或薪資條/袋"},
            {"type": "manual", "label": "💡 額度：符 1 項→5萬/18期、2 項→10萬/24期、3 項→15萬/36期"},
            {"type": "manual", "label": "符項條件（12 選 N）",
             "hint": "1.正職滿 1 年+勞保/薪轉  2.車主/保人軍公教  3.二等親屬保(≤65)或正職保人  "
                     "4.甲乙丙技術士/單一級/大貨大客聯結車職駕/吊掛/3 噸+起重機/護士證且與現職相符  "
                     "5.車主/保人為醫師/建築師/會計師/律師  6.車主/保人上市櫃正職 1 年（附股票代號）  "
                     "7.車主定存/基金/儲蓄險滿 2 年+5 萬+無質押  8.車主/保人自有不動產（無民間私設、申+保持分全部）  "
                     "9.近 15 天內 JCIC（信用正常、不含小白）  10.信用卡滿 1 年+近半年無遲繳（附近一期帳單）  "
                     "11.同業/銀行既有客戶繳滿 10 期+無 15 天以上逾期【借新還舊專案須繳滿 12 期】  "
                     "12.車輛持有滿 1 年且車齡 3 年內"},
            {"type": "simple", "label": "🚫 警示戶不做（自動偵測）", "op": "no_warning_account"},
        ],
        "required_docs": ["身分證正反", "第二證件（健保卡/駕照）", "存摺封面", "機車行照"],
    },
    # ===== 裕融系列（汽車）=====
    # 共同：他行代償/借新還舊/原車融資 都做
    # 鑑價：天書/權威/鑑價 × 1.3 = 可貸金額（例 50萬×1.3=65萬）
    # 沒天書/權威/鑑價 → 老車專案最高 10 萬
    # 警示戶不做（警示戶送和潤）
    # 4 家差別在「符合幾項條件」+ 車齡限制 + 期數
    {
        "company": "裕融新優加",
        "max_amount": 150,
        "priority": 87,
        "rules": [
            {"type": "simple", "label": "中華民國身分證", "field": "id_no", "op": "tw_id", "value": True},
            {"type": "simple", "label": "客戶必須有汽車", "op": "has_car"},
            {"type": "simple", "label": "車齡 ≤ 15 年", "field": "vehicle_year", "op": "year_age_le", "value": 15},
            {"type": "manual", "label": "需符合 3 項條件以上"},
    ...
    },
    {
        ...
        "rules": [
            {"type": "simple", "label": "中華民國身分證", "field": "id_no", "op": "tw_id", "value": True},
            {"type": "simple", "label": "客戶必須有汽車", "op": "has_car"},
            {"type": "simple", "label": "車齡 ≤ 18 年", "field": "vehicle_year", "op": "year_age_le", "value": 18},
            {"type": "manual", "label": "需符合 2 項條件以上"},
            {"type": "manual", "label": "✅ 接受三種情境：他行代償、借新還舊、原車融資"},
    ...
    # ===== 和潤系列（警示戶可送、警示戶優先送和潤）=====
    {
        ...
        "rules": [
            {"type": "simple", "label": "中華民國身分證", "field": "id_no", "op": "tw_id", "value": True},
            {"type": "simple", "label": "客戶必須有汽車", "op": "has_car"},
            {"type": "manual", "label": "需有汽車（無貸款或可加貸都可送）"},
            {"type": "manual", "label": "✅ 接受三種情境：他行代償、借新還舊、原車融資"},
    ...
    },
    {
        ...
        "rules": [
            {"type": "simple", "label": "中華民國身分證", "field": "id_no", "op": "tw_id", "value": True},
            {"type": "simple", "label": "客戶必須有重機（看 eval_property 含「重機」）", "op": "has_heavy_bike"},
            {"type": "manual", "label": "需有重機（無貸款或可加貸都可送）"},
            {"type": "manual", "label": "⚠️ 須先估價/鑑價、確認金額是否足夠才能送"},
            {"type": "manual", "label": "✅ 警示戶可送"},
            {"type": "manual", "label": "⚠️ 警示戶撥款須撥到二等親帳戶"},
        ],
        "required_docs": ["身分證正反", "第二證件（健保卡/駕照）", "存摺封面",
                          "重機行照", "重機鑑價/估價"],
...
# 合法（非私設）資融公司清單 — 動保由這些公司設定的 = 一般車貸、可貸款
LEGIT_LENDERS = [
    # 主流車貸/融資公司（業務常見、名字含這些 keyword 都視為合法）
    "21", "二十一",
    "裕融",
    "和潤",
    "中租", "合迪", "中租迪和",
...
def _is_legit_lender_name(co):
    """公司名是不是合法資融公司（含 LEGIT_LENDERS 或含「銀行」字眼）"""
    if not co:
        return False
    for keyword in LEGIT_LENDERS:
        if keyword in co:
            return True
    if "銀行" in co:
        return True
...
def _is_private_lender_name(co):
    """公司名是不是私設公司
    判斷規則（業務教法）：
    1. 在 PRIVATE_LENDERS 清單裡 → 私設
    2. 名字是當鋪 → 私設
    3. 不在合法資融清單裡、又不是銀行、看不出公司性質 → 視為私設（保守判定）
    """
    if not co:
        return False
    for keyword in PRIVATE_LENDERS:
        if keyword in co:
            return True
    if "當鋪" in co or "當舖" in co:
        return True
    return False


def _customer_has_private_loan(customer):
    """檢查客戶 debt_list 是否有「確認私設」的動保
    黑名單模式：只認 PRIVATE_LENDERS 清單裡的公司、其他不算
    回傳 True = 確認有私設動保（自動標、融資公司不可送、只能送 PRIVATE_OK_PLANS）
    """
    try:
        debt_list = json.loads(customer.get("debt_list") or "[]") if customer.get("debt_list") else []
        for d in debt_list:
...
def _customer_has_legit_loan(customer):
    """檢查客戶 debt_list 是否有任一筆來自合法資融/銀行的動保
    回傳 True = 有合法車貸（可代償/借新還舊）
    """
    try:
        debt_list = json.loads(customer.get("debt_list") or "[]") if customer.get("debt_list") else []
        for d in debt_list:
...
def _customer_unknown_dynbao_companies(customer):
    """找出客戶 debt_list 中「沒見過」的動保公司（不在 PRIVATE_LENDERS 也不在 LEGIT_LENDERS、也不是銀行）
    回傳 list[str] = 沒見過的公司名清單、空 list 表示沒有未知公司
    用途：UI 顯示「⚠️ 動保來自 XX、請人工確認是否私設」
    """
    unknown = []
    try:
        debt_list = json.loads(customer.get("debt_list") or "[]") if customer.get("debt_list") else []
...
def _customer_motorcycle_loan_remaining(customer):
    ...
    重要：dy="無"/空 → 銀行查不到、可當沒貸款送件、不計入前貸
    用途：和裕機車等方案要看「扣完前貸後還可貸 > N 萬」
    """
    import re as _re
    total = 0
    try:
        ...
        for d in debt_list:
            ...
            dy = (d.get("dy", "") or "").strip()
            # 只算有動保/公路設定的（銀行查得到的）
            # 沒動保也沒公路 → 銀行查不到、可當沒貸款送件
            if "動保" not in dy and "公路" not in dy:
                continue
            re_str = (d.get("re", "") or "").strip()
            cleaned = _re.sub(r'[^\d]', '', re_str)
            if cleaned:
                total += int(cleaned)
    except Exception:
        pass
...
def _customer_has_car(customer):
    """檢查客戶是否有汽車
    判定：eval_property 含「汽」字（如「1汽」「1機2汽」）
    或 eval_vehicle 含「汽」/特定車型
    """
    try:
        prop = (customer.get("eval_property", "") or "").strip()
        veh = (customer.get("eval_vehicle", "") or "").strip()
        if "汽" in prop or "汽" in veh:
            return True
    except Exception:
        pass
    return False
...
def _customer_has_heavy_bike(customer):
    """檢查客戶是否有重機
    判定：eval_property 或 eval_vehicle 含「重機」
    """
    try:
        prop = (customer.get("eval_property", "") or "").strip()
        veh = (customer.get("eval_vehicle", "") or "").strip()
        if "重機" in prop or "重機" in veh:
            return True
    except Exception:
        pass
    return False
...
def _customer_has_bank_loan(customer):
    """檢查客戶 debt_list 是否有任何銀行的負債紀錄
    判定：debt_list 中 co 欄位含「銀行」字眼
    用途：第一信貸要求「近一個月內銀行信貸撥款」、沒銀行貸款紀錄 → 不能送
    """
    try:
        debt_list = json.loads(customer.get("debt_list") or "[]") if customer.get("debt_list") else []
        for d in debt_list:
...
def _customer_has_known_contact(customer):
    """檢查客戶聯絡人是否至少有 1 位「知情」
    判定：contact1_known 或 contact2_known 含「知情」、不含「保密」「不知」
    """
    try:
        for k in ["contact1_known", "contact2_known"]:
            v = (customer.get(k, "") or "").strip()
            if not v:
                continue
            if ("不知" in v) or ("保密" in v):
                continue
            if "知情" in v or v in ["是", "有"]:
                return True
    except Exception:
        pass
    return False
...
def _customer_has_house_private(customer):
    """檢查客戶是否有「房屋私設」
    判定：eval_alert 含「有」+ eval_property 含「不動產」
    （eval_alert 「當鋪/私設」標記在「有」、且名下有不動產）
    """
    try:
        alert = (customer.get("eval_alert", "") or "").strip()
        prop = (customer.get("eval_property", "") or "").strip()
        return ("有" in alert) and ("不動產" in prop)
    except Exception:
        return False


# 房屋私設話術（業務操作守則、非自動規則）：
# - 送融資貸款/銀行貸款時 → 一律說「父母名下」、避免私設成為阻擋因素
# - 送房地（如新鑫等房地貸款）時 → 不用、因為房地會去查、講真實情況即可
HOUSE_PRIVATE_SCRIPT_NOTE = "💡 房屋私設話術：送融資/銀行 → 一律說「父母名下」；送房地 → 講真實情況、不用蓋"


# === 新增 auto 判別 helpers (2026/05/05) ===
def _customer_is_warning_account(customer):
    return (customer.get("eval_alert_warning", "") or "").strip() == "是"

def _customer_has_drunk_drug_record(customer):
    text = " ".join((customer.get(k, "") or "") for k in ["eval_law", "eval_note", "eval_credit_card"])
    return any(kw in text for kw in ["酒駕", "酒測", "毒駕", "毒品"])

def _customer_recent_bank_loan(customer):
    """銀行貸款 pa ≤ 1 = 近月撥款"""
    try:
        debts = json.loads(customer.get("debt_list") or "[]") if customer.get("debt_list") else []
        for d in debts:
            if "銀行" not in (d.get("co", "") or ""): continue
            try:
...
def _customer_21group_max_paid(customer):
    ...
    try:
        ...
        for d in debts:
            co = (d.get("co", "") or "")
            if not any(kw in co for kw in GROUP21_KEYWORDS): continue
            try:
                paid = int((d.get("pa", "") or "0").strip())
                if paid > max_paid: max_paid = paid
            except: continue
    except: pass
    return max_paid

def _customer_dynbao_from_apt_legit(customer):
    """動保前貸來自 中租/合迪/和潤/裕融（亞太代償可送）"""
    legit = ["中租", "合迪", "和潤", "裕融"]
    try:
        debts = json.loads(customer.get("debt_list") or "[]") if customer.get("debt_list") else []
        for d in debts:
            if "動保" not in (d.get("dy", "") or ""): continue
            if any(kw in (d.get("co", "") or "") for kw in legit): return True
    except: pass
    return False

def _customer_has_property_no_private(customer):
    """有不動產 + 無房屋私設"""
    prop = (customer.get("eval_property", "") or "").strip()
    if "不動產" not in prop: return False
    if (customer.get("eval_house_private", "") or "").strip() == "有": return False
    return True



# 偵測到「私設」（動保私設 / 當鋪私設 / 房屋私設）時、以下方案仍可送
# 業務教法：融資公司全部不能送、但這些不受私設影響
PRIVATE_OK_PLANS = [
    "銀行",        # 銀行有自己徵信、不看私設
    "零卡(C)",     # 民間、不看條件
    "商品貸",      # 民間
    ...
    "麻吉U質手機",
    "麻吉手機",
    "21商品",      # 21 商品系列、警示戶可送、私設也可送
    "21機車12萬",
    "21機車25萬",  # 注意：21汽車 A/B/C 不在此清單（21汽車有「警示戶不做」紅線）
    "鄉民",        # 民間商品類
    "手機分期",
    "手機無卡分期",
]
...
def _customer_has_road_setting_no_dynbao(customer):
    """檢查客戶有「公路設定」但無「動保設定」
    業務情境：客戶剛貸款（動保還沒登）或 當鋪借款（不會登動保）
    處理：人工問清楚、若是當鋪私設或剛貸款 → 不能送這台車、但其他方案可送
    註：公路設定查詢需車牌+身分證、無法看全車、平常不影響判斷邏輯
    """
    try:
        debt_list = json.loads(customer.get("debt_list") or "[]") if customer.get("debt_list") else []
        has_road = False
        ...
        for d in debt_list:
            ...
            if "動保" in dy:
                has_dynbao = True
        return has_road and not has_dynbao
    except Exception:
        return False
...
def _check_creditcard_status(customer):
    ...
    回傳 (status, detail)
    - "good": 有信用卡、無問題（可作財力）
    - "bad": 有問題（卡循/遲繳/強制停卡/協商）→ 紅線
    - "none": 沒填或沒卡
    """
    text = (customer.get("eval_credit_card", "") or "").strip()
    if not text:
        return ("none", "未填")
    # 紅線關鍵字：表示信用有問題
    bad_keywords = ["卡循", "循環", "遲繳", "強制停卡", "停卡", "協商", "呆帳"]
    for kw in bad_keywords:
        if kw in text:
            return ("bad", f"有「{kw}」狀況：{text}")
    # 「無」「沒有」表示沒卡
    if text in ("無", "沒有", "0", "無卡"):
        return ("none", "沒信用卡")
    # 其他文字（銀行名等）→ 視為有卡且無問題
    return ("good", text)
...
def _check_rule(rule, customer):
    ...
    if rt == "simple":
        ...
        # 特殊 op：客戶有銀行貸款（debt_list 有銀行字眼）
        if op == "has_bank_loan":
            ok = _customer_has_bank_loan(customer)
            return ("pass" if ok else "fail", label, "有銀行貸款" if ok else "無銀行貸款紀錄")
        # 特殊 op：客戶有汽車
        if op == "has_car":
            ok = _customer_has_car(customer)
            return ("pass" if ok else "fail", label, "有汽車" if ok else "無汽車")
        # 特殊 op：客戶有重機
        if op == "has_heavy_bike":
            ok = _customer_has_heavy_bike(customer)
            return ("pass" if ok else "fail", label, "有重機" if ok else "無重機")
        # 特殊 op：機車前貸剩餘 < value 元（用於和裕機車「扣完前貸還可貸>10萬」）
        if op == "motorcycle_loan_remaining_lt":
            remain = _customer_motorcycle_loan_remaining(customer)
            threshold = float(value)
            if remain < threshold:
                return ("pass", label, f"機車前貸剩 {remain:,} 元")
            return ("fail", label, f"機車前貸剩 {remain:,} 元（≥ {int(threshold):,}、扣完前貸不夠 10 萬）")
        # 警示戶不做（eval_alert_warning="是" → fail）
        if op == "no_warning_account":
            if _customer_is_warning_account(customer):
                return ("fail", label, "客戶為警示戶")
            return ("pass", label, "非警示戶")
        # 無酒駕/毒駕紀錄
        if op == "no_drunk_drug":
            if _customer_has_drunk_drug_record(customer):
                return ("fail", label, "法學/備註含酒駕或毒駕紀錄")
            return ("pass", label, "無酒駕/毒駕紀錄")
        # 近月銀行撥款（銀行貸款 pa ≤ 1）
        if op == "recent_bank_loan":
        ...
        if op == "group21_paid_ge":
            ...
            threshold = int(value)
            if paid >= threshold:
                return ("pass", label, f"21 體系最大繳 {paid} 期")
            return ("fail", label, f"21 體系最大繳 {paid} 期（< {threshold}）")
        # 亞太代償專案：動保前貸來自中租/合迪/和潤/裕融
        if op == "dynbao_from_apt_legit":
            if _customer_dynbao_from_apt_legit(customer):
                return ("pass", label, "動保前貸來自合法資融")
            # 沒動保 = 不在代償情境、不擋
            if not _customer_has_dynbao(customer):
                return ("pass", label, "無動保（非代償情境）")
            return ("fail", label, "動保前貸非中租/和潤/裕融、不能代償")
        # 有不動產 + 無房屋私設
        if op == "has_property_no_private":
            if _customer_has_property_no_private(customer):
                return ("pass", label, "有不動產且無私設")
            prop = (customer.get("eval_property", "") or "").strip()
            hp = (customer.get("eval_house_private", "") or "").strip()
            if "不動產" not in prop:
                return ("fail", label, "未持有不動產")
            return ("fail", label, f"不動產有私設（{hp}）")
        # 特殊 op：聯絡人至少 1 位知情
        if op == "any_contact_known":
            ok = _customer_has_known_contact(customer)
            c1 = (customer.get("contact1_known", "") or "").strip() or "未填"
            c2 = (customer.get("contact2_known", "") or "").strip() or "未填"
            return ("pass" if ok else "fail", label, f"聯絡人1：{c1}、聯絡人2：{c2}")
        ...
        if op == "creditcard_good":
            ...
            if status == "good":
                ...
                return ("unknown", label, "未填或無卡")
        if op == "creditcard_no_bad":
            # 只要無紅線（卡循/遲繳/停卡/協商）就 pass、可以無卡
            status, detail = _check_creditcard_status(customer)
            if status == "bad":
                return ("fail", label, detail)
            return ("pass", label, detail if status == "good" else "無紅線")
        # 取值
        if field == "age":
        ...
        try:
            if op == "between":
                ...
                # 數字比較、空值視為 0
                try:
                    ok = float(actual or 0) >= float(value)
                except Exception:
                    ok = False
                actual_str = str(actual)
            elif op == "<=":
...
def evaluate_case(customer):
    """跑所有規則、回排序好的方案清單 + 私設狀態

    回傳格式：{
      "results": [...],            # 每家方案的判別結果
      "private_status": "confirmed"|None,  # confirmed = 確認私設、None = 沒問題
      "unknown_dynbao": [...],     # 沒見過的動保公司清單（要人工確認）
      "house_private": True/False, # 房屋私設標記（送融資/銀行話術提示）
    }
    """
    # 偵測私設狀態
    has_private = _customer_has_private_loan(customer)
    unknown_dynbao = _customer_unknown_dynbao_companies(customer)
    has_house_private = _customer_has_house_private(customer)
    # 機車前貸剩餘（單位：元）— 全機車方案套：max_amount - 前貸 < 10萬 → 自動 fail
    moto_remain = _customer_motorcycle_loan_remaining(customer)

    results = []
    for plan in PLAN_ELIGIBILITY_RULES:
        checks = []
        has_simple_fail = False  # 硬條件失敗（年齡/身分證/有汽車 等）
        ...
        for rule in plan["rules"]:
            ...
            if status == "fail":
                rt = rule.get("type", "")
                if rt == "simple":
                    has_simple_fail = True
                elif rt == "oneof":
                    has_oneof_fail = True
                else:
                    has_simple_fail = True  # 其他類型 fail 視為硬條件
            elif status in ("manual", "unknown"):
                manual_count += 1

        # 私設處理：確認私設 + 不在 PRIVATE_OK_PLANS → 強制標 fail
        private_blocked = False
        if has_private and plan["company"] not in PRIVATE_OK_PLANS:
            private_blocked = True
            checks.append({
                "status": "fail",
                "label": "🚫 客戶有確認私設動保、本方案不可送",
                "actual": "私設動保",
            })
            has_simple_fail = True

        # 機車前貸額度檢查（適用所有名稱含「機車」的方案）
        # 規則：max_amount - 機車前貸剩餘 必須 > 10 萬（業務鐵則）
        if moto_remain > 0 and "機車" in plan["company"]:
            margin = plan["max_amount"] * 10000 - moto_remain
            if margin < 100000:
                checks.append({
                    "status": "fail",
                    "label": f"🚫 機車前貸剩 {moto_remain:,} 元、扣完後可貸 {margin:,} 元 < 10 萬",
                    "actual": "前貸過高、無法送機車方案",
                })
                has_simple_fail = True

        # 房屋私設話術提示（融資/銀行類）
        if has_house_private and plan["company"] in PRIVATE_OK_PLANS:
            if plan["company"] in ["銀行", "鄉民"]:
                checks.append({
                    "status": "manual",
                    "label": "💡 客戶有房屋私設、話術說「父母名下」",
                    "actual": "需話術",
                })
                manual_count += 1
        ...
        if has_simple_fail:
            ...
            if private_blocked:
                ...
                summary = "❌ 不符合（看 ❌ 項目）"
        elif plan.get("always_eligible"):
            # 固定排的方案（鄉民/民間 4 家）：simple 通過就標 ✅、oneof/manual 不擋
            eligible = True
            summary = "✅ 固定排（每個客戶都會送）"
        elif has_oneof_fail:
            eligible = False
    ...
    return {
        "results": results,
        "private_status": "confirmed" if has_private else None,
        "unknown_dynbao": unknown_dynbao,
        "house_private": has_house_private,
    }
...
def _build_pdf_from_images(images: list, name_hint: str = "") -> tuple:
    ...
    safe_name = re.sub(r"[^\w㐀-鿿豈-﫿]", "_", name_hint or "收件")[:30]
...
def extract_status_summary(first_line: str, customer_name: str) -> str:
    ...
    if "/" in text and re.search(r"[\u3400-\u9fff\uf900-\ufaff0-9]+/[\u3400-\u9fff\uf900-\ufaff0-9]+", text):
        return text[:40]
...
COMPANY_SECTION_MAP = {
    ...
    "亞太機車": "亞太", "亞太機": "亞太",
    # 21方案 → 機車/商品/25萬 歸到「21」欄位、但 21汽車獨立區塊
    "21機車": "21",
    "21機": "21", "21機25": "21", "21機25萬": "21",
    "21機車12萬": "21", "21機車25萬": "21",
    # 21汽車 / 21汽 不歸到 21、保留各自獨立區塊（REPORT_SECTION_1 已含「21汽車」）
    "21汽": "21汽車",
    # 分貝方案 → 各自欄位
    "分貝汽": "分貝汽車",
    "分貝機": "分貝機車",
...
def parse_special_command(text: str, group_id: str) -> Optional[Dict]:
    ...
    if m:
        return {"type": "cancel_disbursement", "name": m.group(1), "company": m.group(2).strip()}
    ...
    # 結案帶原因：@AI 姓名 結案 原因
    m = re.match(r"^([㐀-鿿豈-﫿]{2,6})\s*結案\s+(.+)$", clean)
    if m:
        return {"type": "close", "name": m.group(1), "reason": m.group(2).strip()}

    # 結案
    m = re.match(r"^([㐀-鿿豈-﫿]{2,6})\s*(已結案|結案)$", clean)
    if m:
    ...
    m = re.match(r"^([㐀-鿿豈-﫿]{2,6})\s+(.+?)\s*結案$", clean)
    if m:
    ...
    m = re.match(r"^([㐀-鿿豈-﫿]{2,6})\s+(.+?)\s*婉拒\s*送\s*([^\s]+)\s*$", clean)
    if m:
    ...
    # 婉拒（推到下一家）
    m = re.match(r"^([㐀-鿿豈-﫿]{2,6})\s*婉拒$", clean)
    if m:
        return {"type": "reject", "name": m.group(1)}

    # 確認婉拒（房地/銀行/C 類別的二次確認指令）：@AI 姓名 公司 確認婉拒
    m = re.match(r"^([㐀-鿿豈-﫿]{2,6})\s+(.+?)\s*確[定認]婉拒$", clean)
    if m:
    ...
    m = re.match(r"^([㐀-鿿豈-﫿]{2,6})\s+(.+?)\s*婉拒$", clean)
    if m:
    ...
    def _parse_penalty_amt(num_str, is_wan):
        try:
            ...
            return num_str
    # 違約金確認支付 — 第二階段正式結案
    m = re.match(r"^([㐀-鿿豈-﫿]{2,6})\s*違約金(?:確認支付|支付確認)\s*(\d+(?:\.\d+)?)\s*(萬)?", clean)
    if m:
    ...
    m = re.match(r"^([㐀-鿿豈-﫿]{2,6})\s*違約金已支付\s*(\d+(?:\.\d+)?)\s*(萬)?", clean)
    if m:
        amt = _parse_penalty_amt(m.group(2), bool(m.group(3)))
        return {"type": "penalty", "name": m.group(1), "penalty": amt, "confirmed": False}
    # Fallback：舊格式 含逗號數字
    m = re.match(r"^([㐀-鿿豈-﫿]{2,6})\s*違約金已支付\s*([\d,，]+)", clean)
    if m:
    ...
    def _strip_verb(s):
        ...
        for item in re.split(r"([+＋])", s):
            if item in ("+", "＋"): out.append(item); continue
            item = re.sub(r"^\s*(?:送到|轉到|送|轉)\s*", "", item)
            out.append(item)
...
def generate_notification_text(r: dict, company: str = "") -> str:
    ...
    # 找 PLAN_INFO：先直查、再試 COMPANY_ALIAS、最後試 normalize_section
    plan_info = PLAN_INFO.get(co)
    if not plan_info and co:
        aliased = COMPANY_ALIAS.get(co, "")
        if aliased:
            plan_info = PLAN_INFO.get(aliased)
    if not plan_info and co:
        try:
            norm = normalize_section(co)
            plan_info = PLAN_INFO.get(norm)
        except Exception:
            pass
    if manual_amount:
...
【各家獨立缺件】多家同送、要分別記每家缺什麼
  指定公司缺：@AI 王小明 亞太 缺 勞保
  指定公司已補：@AI 王小明 亞太 已補 勞保
  情境：多家同送（如亞太+喬美）、亞太缺勞保、喬美缺繳息
       打 @AI 王小明 亞太 缺 勞保 → 只動亞太
       打 @AI 王小明 喬美 缺 繳息 → 只動喬美
  防呆：多家同送沒指定公司就打「@AI 王小明 缺 XX」會被擋下、要求指定哪家
...
【收件 PDF】把多張圖合成 PDF 拿下載連結
  開始：@AI 王小明 收件          （或 @AI 收件 不指定姓名）
  傳圖：[圖1] [圖2] [圖3]         （直接傳、BOT 安靜接收、不會吵）
  完成：@AI 王小明 完成           （或等 5 分鐘自動合）
  取消：@AI 王小明 取消
  說明：BOT 合 PDF 推群組下載連結、24 小時後自動失效
       連結業務手機點開直接下載"""
    ...
    if t == "cancel_disbursement":
        ...
        if not company:
            reply_text(reply_token,
                       f"⚠️ 要取消哪家的撥款？\n例：@AI {name} 和裕 取消撥款")
            return
        target = _resolve_target_strict(cmd, name, group_id, reply_token, "取消撥款")
        if not target:
            return
        if not _check_active_or_warn(target, reply_token, "取消撥款", name):
            return
        company = COMPANY_ALIAS.get(company, company)
        co_norm = normalize_section(company)
        route = target["route_plan"] or ""
        data = parse_route_json(route) if route else {"order":[], "current_index":0, "history":[]}
        history = data.get("history", [])
        # 把該家的「撥款」狀態降回「核准」、清掉 disbursed 欄位
        changed = False
        for h2 in history:
            hc = h2.get("company", "")
            if normalize_section(hc) == co_norm and h2.get("status") == "撥款":
                h2["status"] = "核准"
                h2.pop("disbursed", None)
                changed = True
        data["history"] = history
        new_route = json.dumps(data, ensure_ascii=False)
        # 客戶 disbursement_date 欄位清空（如果該家是 current 或唯一已撥）
        # 判斷：history 內還有沒有其他「撥款」狀態 → 沒有 → 清空 disbursement_date
        any_disbursed = any(h2.get("status") == "撥款" for h2 in history)
        new_disb = "" if not any_disbursed else target["disbursement_date"]
        update_customer(target["case_id"], route_plan=new_route,
                        disbursement_date=new_disb,
                        report_section="待撥款",  # 回到待撥款區塊
                        text=f"{name} {company} 取消撥款", from_group_id=group_id)
        push_text(target["source_group_id"], f"{name} {company} 取消撥款")
        if changed:
            reply_text(reply_token,
                       f"✅ {name} 已取消 {company} 撥款\n"
                       f"核准記錄保留、回到「待撥款」區塊\n"
                       f"其他家不受影響、繼續送件中")
        else:
            reply_text(reply_token,
                       f"⚠️ 沒找到 {name} 的 {company} 撥款記錄\n"
                       f"可能 {company} 還沒撥款、或公司名拼錯")
        return
        ...
        if removed or disbursed_archived_co:
            ...
            if disbursed_archived_co:
                ...
                if promoted_co:
                    ...
                    # idx 修正：若舊 idx 指向的公司被刪、idx 重指 current
...
def _process_event_inner(event: dict):
    ...
    if has_ai_trigger(text):
        ...
        m_pdf_done = re.match(r"^([㐀-鿿豈-﫿]{2,8}\s+)?(完成|取消)$", clean)
        if m_pdf_done:
            ...
            return
        # @AI 收件 / @AI 姓名 收件
        m_pdf_start = re.match(r"^(?:([㐀-鿿豈-﫿]{2,8})\s+)?收件$", clean)
        if m_pdf_start:
...
# ========== 勞工優選 4 月匯入 ==========
YOUSHUAN_GROUP_ID = "C58db10759b2b4e36dc46f1e6293d968f"
YOUSHUAN_GROUP_NAME = "勞工優選"


@app.get("/admin/import-youshuan", response_class=HTMLResponse)
def import_youshuan_preview(request: Request):
    from fastapi.responses import RedirectResponse
    role = check_auth(request)
    if role != "admin":
        return RedirectResponse("/login")
    _base = os.path.dirname(os.path.abspath(__file__))
    jpath = os.path.join(_base, "youshuan_apr_import.json")
    if not os.path.exists(jpath):
        return HTMLResponse("<div style='padding:40px'>找不到 youshuan_apr_import.json</div>")
    with open(jpath, encoding="utf-8") as f:
        data = json.load(f)
    rows_html = ""
    for d in data:
        ...
        rows_html += f'''<tr>
          ...
          <td style="padding:6px 10px">{h(amt_str)}</td>
          <td style="padding:6px 10px">{h(disb_str)}</td>
          <td style="padding:6px 10px;font-size:11px;color:#6b7280">{h(d.get("report_note","-") or "-")}</td>
        </tr>'''
    return HTMLResponse(f"""<!DOCTYPE html><html><head>{PAGE_CSS}<title>匯入 4 月勞工優選案件</title></head><body>
    {make_topnav(role, "admin")}
    <div class="page">
      <h2>匯入 4 月勞工優選案件（共 {len(data)} 筆）</h2>
      <div style="font-size:13px;color:#6b7280;margin-bottom:12px">
        目標業務群：<b>勞工優選</b>（{YOUSHUAN_GROUP_ID}）<br>
        身分證已存在的會自動跳過、不會重複建案。<br>
        群組未存在會自動建立為 SALES_GROUP。<br>
        ⚠️ 此匯入<b>只比對身分證</b>、不再用姓名 fallback。
      </div>
      <div style="background:#fff;border:1px solid #e5e7eb;border-radius:10px;overflow:auto;margin-bottom:16px">
        <table style="width:100%;border-collapse:collapse;font-size:13px">
          <thead style="background:#f9fafb;border-bottom:1px solid #e5e7eb;position:sticky;top:0">
      ...
      <div style="display:flex;gap:12px;align-items:center">
        <form method="post" action="/admin/import-youshuan/confirm" onsubmit="return confirm('確定匯入 {len(data)} 筆到「勞工優選」業務群嗎？')">
          <button type="submit" class="btn btn-primary" style="padding:10px 24px;font-size:15px">✅ 確認匯入 {len(data)} 筆</button>
        </form>
        <form method="post" action="/admin/clear-group" onsubmit="return confirm('確定要清空「勞工優選」群的所有客戶嗎？')">
          <input type="hidden" name="group_id" value="{YOUSHUAN_GROUP_ID}">
          <button type="submit" style="padding:10px 18px;font-size:14px;background:#b91c1c;color:#fff;border:none;border-radius:7px;cursor:pointer">🗑 先清空勞工優選群</button>
        </form>
      </div>
    </div></body></html>""")


@app.post("/admin/import-youshuan/confirm")
async def import_youshuan_confirm(request: Request):
    role = check_auth(request)
    if role != "admin":
        return HTMLResponse("無權限", status_code=403)
    _base = os.path.dirname(os.path.abspath(__file__))
    ...
    SALES_GROUP_ID = YOUSHUAN_GROUP_ID
    with db_conn(commit=True) as conn:
        cur = conn.cursor()
        ...
        if not cur.fetchone():
            cur.execute(
                ...
                (SALES_GROUP_ID, YOUSHUAN_GROUP_NAME, "SALES_GROUP"),
            )
    ...
    for d in data:
        ...
        try:
            ...
            if existing:
                ...
                concurrent_str = ",".join(d.get("concurrent", []))
                merge_kwargs = {"source_group_id": SALES_GROUP_ID,
                                "text": f"已從【{old_gname}】搬到【{YOUSHUAN_GROUP_NAME}】",
                                "from_group_id": SALES_GROUP_ID}
                if approved and not (existing["approved_amount"] or "").strip():
                    merge_kwargs["approved_amount"] = approved
                if disb and not (existing["disbursement_date"] or "").strip():
            ...
            approved = (d.get("approved_amount") or "").strip()
            disb = (d.get("disb_date") or "").strip()
            extra_approved = d.get("extra_approved", [])
            cust_status = d.get("status", "ACTIVE")
            company_notes = d.get("company_notes") or {}
            ...
            if cust_status == "PENALTY":
                with db_conn(commit=True) as conn:
                    cur = conn.cursor()
                    cur.execute("UPDATE customers SET status='PENALTY' WHERE case_id=?", (case_id,))
            ...
            if update_kwargs:
                update_customer(case_id, text=None, from_group_id=SALES_GROUP_ID, **update_kwargs)
            # 寫 company_status（每家公司獨立備註、日報該家區塊用）
            if company_notes:
                try:
                    cs_json = json.dumps(company_notes, ensure_ascii=False)
                    with db_conn(commit=True) as conn:
            ...
            if extra_fields:
                with db_conn(commit=True) as conn:
                    ...
                    vals = list(extra_fields.values()) + [case_id]
                    cur.execute(f"UPDATE customers SET {set_clause} WHERE case_id=?", vals)
            created.append(name + ("(違約金結案)" if cust_status == "PENALTY" else ""))
        except Exception as e:
            failed.append(f"{name}：{e}")
    ...
    summary = f"""<div style="padding:40px;font-family:sans-serif">
      ...
      <p>🔀 搬到勞工優選群：{len(transferred)} 筆 — {', '.join(transferred)}</p>
      <p>⚠️ 跳過：{len(skipped)} 筆 — {', '.join(skipped)}</p>
      <p>❌ 失敗：{len(failed)} 筆 — {'; '.join(failed)}</p>
...
# ========== 樂保 4 月匯入 ==========
LEBAO_GROUP_ID = "C59054bd9fd4e08636cbf4497dbc35f32"
LEBAO_GROUP_NAME = "樂保"


@app.get("/admin/import-lebao", response_class=HTMLResponse)
def import_lebao_preview(request: Request):
    """從 lebao_apr_import.json 讀客戶資料、預覽匯入頁（admin 限定）。"""
    from fastapi.responses import RedirectResponse
    role = check_auth(request)
    if role != "admin":
        return RedirectResponse("/login")
    _base = os.path.dirname(os.path.abspath(__file__))
    jpath = os.path.join(_base, "lebao_apr_import.json")
    if not os.path.exists(jpath):
        return HTMLResponse("<div style='padding:40px'>找不到 lebao_apr_import.json</div>")
    with open(jpath, encoding="utf-8") as f:
        data = json.load(f)
    rows_html = ""
    for d in data:
    ...
    return HTMLResponse(f"""<!DOCTYPE html><html><head>{PAGE_CSS}<title>匯入 4 月樂保案件</title></head><body>
    {make_topnav(role, "admin")}
    <div class="page">
      <h2>匯入 4 月樂保案件（共 {len(data)} 筆、含 1 違約金結案）</h2>
      <div style="font-size:13px;color:#6b7280;margin-bottom:12px">
        目標業務群：<b>樂保</b>（{LEBAO_GROUP_ID}）<br>
        身分證已存在的會自動跳過、不會重複建案。<br>
        群組未存在會自動建立為 SALES_GROUP。<br>
        ⚠️ 此匯入<b>只比對身分證</b>、不再用姓名 fallback（避免誤搬其他群組的同名客戶）。
      </div>
      <div style="background:#fff;border:1px solid #e5e7eb;border-radius:10px;overflow:auto;margin-bottom:16px">
        <table style="width:100%;border-collapse:collapse;font-size:13px">
          <thead style="background:#f9fafb;border-bottom:1px solid #e5e7eb;position:sticky;top:0">
      ...
      <div style="display:flex;gap:12px;align-items:center">
        <form method="post" action="/admin/import-lebao/confirm" onsubmit="return confirm('確定匯入 {len(data)} 筆到「樂保」業務群嗎？')">
          <button type="submit" class="btn btn-primary" style="padding:10px 24px;font-size:15px">✅ 確認匯入 {len(data)} 筆</button>
        </form>
        <form method="post" action="/admin/clear-group" onsubmit="return confirm('確定要清空「樂保」群的所有客戶嗎？（匯入失誤時重來用）')">
          <input type="hidden" name="group_id" value="{LEBAO_GROUP_ID}">
          <button type="submit" style="padding:10px 18px;font-size:14px;background:#b91c1c;color:#fff;border:none;border-radius:7px;cursor:pointer">🗑 先清空樂保群</button>
        </form>
      </div>
    </div></body></html>""")


@app.post("/admin/import-lebao/confirm")
async def import_lebao_confirm(request: Request):
    role = check_auth(request)
    if role != "admin":
        return HTMLResponse("無權限", status_code=403)
    _base = os.path.dirname(os.path.abspath(__file__))
    ...
    with db_conn(commit=True) as conn:
        ...
        if not cur.fetchone():
            cur.execute(
                "INSERT INTO groups (group_id, group_name, group_type, is_active) VALUES (?, ?, ?, 1)",
                (SALES_GROUP_ID, LEBAO_GROUP_NAME, "SALES_GROUP"),
            )
    ...
    for d in data:
        ...
        id_no = (d.get("id_no") or "").strip()
        try:
            # 樂保只比對身分證、不用姓名 fallback（避免誤搬其他群組同名客戶）
            existing = find_active_by_id_no(id_no) if id_no else None
            if existing:
                old_gid = existing["source_group_id"]
                if old_gid == SALES_GROUP_ID:
                    skipped.append(f"{name}（已在樂保群）")
                    continue
                old_gname = get_group_name(old_gid) or old_gid[:8]
                approved = (d.get("approved_amount") or "").strip()
                disb = (d.get("disb_date") or "").strip()
                ...
                concurrent_str = ",".join(d.get("concurrent", []))
                merge_kwargs = {"source_group_id": SALES_GROUP_ID,
                                "text": f"已從【{old_gname}】搬到【{LEBAO_GROUP_NAME}】",
                                "from_group_id": SALES_GROUP_ID}
                if approved and not (existing["approved_amount"] or "").strip():
                    merge_kwargs["approved_amount"] = approved
                if disb and not (existing["disbursement_date"] or "").strip():
    ...
    summary = f"""<div style="padding:40px;font-family:sans-serif">
      <h2>樂保匯入完成</h2>
      <p>✅ 新建：{len(created)} 筆 — {', '.join(created[:30])}{'...' if len(created)>30 else ''}</p>
      <p>🔀 搬到樂保群：{len(transferred)} 筆 — {', '.join(transferred)}</p>
      <p>⚠️ 跳過：{len(skipped)} 筆 — {', '.join(skipped)}</p>
      <p>❌ 失敗：{len(failed)} 筆 — {'; '.join(failed)}</p>
      <a href="/admin/groups">回群組管理</a> | <a href="/history">看歷史</a> | <a href="/report">看日報</a>
    </div>"""
...
# ========== 幸福貸 4 月匯入（鉅烽團隊新群） ==========
XINFU_GROUP_ID = "C1846d7a64cd003c41ede87b921121fe4"
XINFU_GROUP_NAME = "幸福"


@app.get("/admin/import-xinfu", response_class=HTMLResponse)
def import_xinfu_preview(request: Request):
    from fastapi.responses import RedirectResponse
    role = check_auth(request)
    if role != "admin":
        return RedirectResponse("/login")
    _base = os.path.dirname(os.path.abspath(__file__))
    jpath = os.path.join(_base, "xinfu_apr_import.json")
    if not os.path.exists(jpath):
        return HTMLResponse("<div style='padding:40px'>找不到 xinfu_apr_import.json — 請先在本機跑 _import_xinfu_parse.py</div>")
    with open(jpath, encoding="utf-8") as f:
        data = json.load(f)
    rows_html = ""
    for d in data:
        ...
        amt_str = d.get("approved_amount", "") or ""
        disb_str = d.get("disb_date", "") or ""
        if d.get("report_section") == "待撥款":
            tag = "💰待撥款"
        elif d.get("report_section") == "送件":
            tag = "📤送件"
        elif d.get("report_section", "").startswith("核准"):
            tag = "✅已核准"
        else:
            tag = ""
        rows_html += f'''<tr>
    ...
    return HTMLResponse(f"""<!DOCTYPE html><html><head>{PAGE_CSS}<title>匯入幸福貸 4 月案件</title></head><body>
    {make_topnav(role, "admin")}
    <div class="page">
      <h2>匯入幸福貸 4 月案件（共 {len(data)} 筆）</h2>
      <div style="font-size:13px;color:#6b7280;margin-bottom:12px">
        目標業務群：<b>幸福</b>（{XINFU_GROUP_ID}）<br>
        身分證已存在於別群會搬到幸福群、本群有的跳過、新的建立。<br>
        群組未存在會自動建立為 SALES_GROUP。
      </div>
      <div style="background:#fff;border:1px solid #e5e7eb;border-radius:10px;overflow:auto;margin-bottom:16px">
        <table style="width:100%;border-collapse:collapse;font-size:13px">
      ...
      <div style="display:flex;gap:12px;align-items:center">
        <form method="post" action="/admin/import-xinfu/confirm" onsubmit="return confirm('確定匯入 {len(data)} 筆到「幸福」業務群嗎？')">
          <button type="submit" class="btn btn-primary" style="padding:10px 24px;font-size:15px">✅ 確認匯入 {len(data)} 筆</button>
        </form>
        <form method="post" action="/admin/clear-group" onsubmit="return confirm('確定要清空「幸福」群的所有客戶嗎？')">
          <input type="hidden" name="group_id" value="{XINFU_GROUP_ID}">
          <button type="submit" style="padding:10px 18px;font-size:14px;background:#b91c1c;color:#fff;border:none;border-radius:7px;cursor:pointer">🗑 先清空幸福群</button>
        </form>
      </div>
    </div></body></html>""")


@app.post("/admin/import-xinfu/confirm")
async def import_xinfu_confirm(request: Request):
    role = check_auth(request)
    if role != "admin":
        return HTMLResponse("無權限", status_code=403)
    _base = os.path.dirname(os.path.abspath(__file__))
    ...
    with db_conn(commit=True) as conn:
        ...
        if not cur.fetchone():
            cur.execute(
                "INSERT INTO groups (group_id, group_name, group_type, is_active) VALUES (?, ?, ?, 1)",
                (SALES_GROUP_ID, XINFU_GROUP_NAME, "SALES_GROUP"),
            )
    created, transferred, skipped, failed = [], [], [], []
    for d in data:
        name = d["name"]
        ...
        try:
            ...
            if existing:
                ...
                concurrent_str = ",".join(d.get("concurrent", []))
                merge_kwargs = {"source_group_id": SALES_GROUP_ID,
                                "text": f"已從【{old_gname}】搬到【{XINFU_GROUP_NAME}】",
                                "from_group_id": SALES_GROUP_ID}
                if approved and not (existing["approved_amount"] or "").strip():
                    merge_kwargs["approved_amount"] = approved
                if disb and not (existing["disbursement_date"] or "").strip():
    ...
    summary = f"""<div style="padding:40px;font-family:sans-serif">
      <h2>幸福貸匯入完成</h2>
      <p>✅ 新建：{len(created)} 筆 — {', '.join(created[:30])}{'...' if len(created)>30 else ''}</p>
      <p>🔀 搬到幸福群：{len(transferred)} 筆 — {', '.join(transferred)}</p>
      <p>⚠️ 跳過：{len(skipped)} 筆 — {', '.join(skipped)}</p>
      <p>❌ 失敗：{len(failed)} 筆 — {'; '.join(failed)}</p>
      <a href="/admin/groups">回群組管理</a> | <a href="/history">看歷史</a> | <a href="/report">看日報</a>
    </div>"""
...
# ========== 優勢（鈔優勢貸）5 月匯入 ==========
CHAO_GROUP_ID = "C81cbefc04111cba498345ea7cc99dece"
CHAO_GROUP_NAME = "優勢"


@app.get("/admin/import-chao", response_class=HTMLResponse)
def import_chao_preview(request: Request):
    from fastapi.responses import RedirectResponse
    role = check_auth(request)
    if role != "admin":
        return RedirectResponse("/login")
    _base = os.path.dirname(os.path.abspath(__file__))
    jpath = os.path.join(_base, "chao_may_import.json")
    if not os.path.exists(jpath):
        return HTMLResponse("<div style='padding:40px'>找不到 chao_may_import.json</div>")
    with open(jpath, encoding="utf-8") as f:
        data = json.load(f)
    rows_html = ""
    for d in data:
    ...
    return HTMLResponse(f"""<!DOCTYPE html><html><head>{PAGE_CSS}<title>匯入優勢 5 月案件</title></head><body>
    {make_topnav(role, "admin")}
    <div class="page">
      <h2>匯入優勢 5 月案件（共 {len(data)} 筆）</h2>
      <div style="font-size:13px;color:#6b7280;margin-bottom:12px">
        目標業務群：<b>優勢</b>（{CHAO_GROUP_ID}）<br>
        身分證已存在於別群會搬到優勢群、本群有的跳過、新的建立。<br>
        群組未存在會自動建立為 SALES_GROUP。
      </div>
      <div style="background:#fff;border:1px solid #e5e7eb;border-radius:10px;overflow:auto;margin-bottom:16px">
        <table style="width:100%;border-collapse:collapse;font-size:13px">
      ...
      <div style="display:flex;gap:12px;align-items:center">
        <form method="post" action="/admin/import-chao/confirm" onsubmit="return confirm('確定匯入 {len(data)} 筆到「優勢」業務群嗎？')">
          <button type="submit" class="btn btn-primary" style="padding:10px 24px;font-size:15px">✅ 確認匯入 {len(data)} 筆</button>
        </form>
        <form method="post" action="/admin/clear-group" onsubmit="return confirm('確定要清空「優勢」群的所有客戶嗎？')">
          <input type="hidden" name="group_id" value="{CHAO_GROUP_ID}">
          <button type="submit" style="padding:10px 18px;font-size:14px;background:#b91c1c;color:#fff;border:none;border-radius:7px;cursor:pointer">🗑 先清空優勢群</button>
        </form>
      </div>
    </div></body></html>""")


@app.post("/admin/import-chao/confirm")
async def import_chao_confirm(request: Request):
    role = check_auth(request)
    if role != "admin":
        return HTMLResponse("無權限", status_code=403)
    _base = os.path.dirname(os.path.abspath(__file__))
    ...
    with db_conn(commit=True) as conn:
        ...
        if not cur.fetchone():
            cur.execute(
                "INSERT INTO groups (group_id, group_name, group_type, is_active) VALUES (?, ?, ?, 1)",
                (SALES_GROUP_ID, CHAO_GROUP_NAME, "SALES_GROUP"),
            )
    created, transferred, skipped, failed = [], [], [], []
    for d in data:
        name = d["name"]
        ...
        try:
            ...
            if existing:
                ...
                concurrent_str = ",".join(d.get("concurrent", []))
                merge_kwargs = {"source_group_id": SALES_GROUP_ID,
                                "text": f"已從【{old_gname}】搬到【{CHAO_GROUP_NAME}】",
                                "from_group_id": SALES_GROUP_ID}
                if approved and not (existing["approved_amount"] or "").strip():
                    merge_kwargs["approved_amount"] = approved
                if disb and not (existing["disbursement_date"] or "").strip():
            ...
            if approved_history:
                ...
                if disb:
                    main_h["disbursed"] = disb
                history.append(main_h)
                route_plan = make_route_json([co], 0, history)
            case_id = create_customer_record(
                name=name, id_no=id_no, company=co,
                source_group_id=SALES_GROUP_ID, text=text,
                route_plan=route_plan, current_company=co,
    ...
    summary = f"""<div style="padding:40px;font-family:sans-serif">
      <h2>優勢匯入完成</h2>
      <p>✅ 新建：{len(created)} 筆 — {', '.join(created[:30])}{'...' if len(created)>30 else ''}</p>
      <p>🔀 搬到優勢群：{len(transferred)} 筆 — {', '.join(transferred)}</p>
      <p>⚠️ 跳過：{len(skipped)} 筆 — {', '.join(skipped)}</p>
      <p>❌ 失敗：{len(failed)} 筆 — {'; '.join(failed)}</p>
      <a href="/admin/groups">回群組管理</a> | <a href="/history">看歷史</a> | <a href="/report">看日報</a>
    </div>"""
...
def download_pdf(token: str, filename: str):
    ...
    safe_fn = re.sub(r"[^\w㐀-鿿豈-﫿\.\-_]", "_", filename)[:80]
...
async def restore_from_report_post(request: Request):
    ...
    for raw_line in text.splitlines():
        ...
        m = re.match(r"^[🆕💰⏰\s]*(\d{1,2}/\d{1,2})-([㐀-鿿豈-﫿]{2,8})(?P<sep>[-\(])(?P<rest>.*)$", line)
        if not m:
        ...
        if cur_section == "待撥款":
            ...
            # 先試 C/D/E：公司在「核准」前面
            m_pre = re.match(r"^(?:轉)?([㐀-鿿豈-﫿0-9]+?)\s*-?\s*核准\s*(\d+(?:\.\d+)?)萬\s*(?:\(([^)]*)\))?", rest)
            if m_pre:
                ...
                if am_full:
                    ...
                    for sm in re.finditer(r"([㐀-鿿豈-﫿0-9]+?)\s*(\d+(?:\.\d+)?)萬\s*\(([^)]+)\)", tail):
                    ...
                    if not approvals:
                        sm2 = re.match(r"([㐀-鿿豈-﫿0-9]+?)\s*(\d+(?:\.\d+)?)萬", tail)
                        if sm2:
...
def report_web(request: Request):
    ...
    function deleteFromReport(caseId){{
      ...
      var i1=document.createElement('input');i1.type='hidden';i1.name='case_id';i1.value=caseId;f.appendChild(i1);
      var i2=document.createElement('input');i2.type='hidden';i2.name='from';i2.value='report';f.appendChild(i2);
...
.ab-card{overflow-x:auto}
    ...
    <div class="ab-card">
      ...
      <div class="ab-block" data-plans="分貝汽車,分貝機車" style="background:#ece8e2;">
        ...
        <textarea name="bei_note" class="ab-inp" placeholder="依上方題目填答、例：&#10;1. 有送過21世紀&#10;2. 沒過件&#10;3. 有貸款&#10;4. 10萬&#10;5. 沒遲繳" style="min-height:120px;resize:vertical;">{h(customer.get('adminb_bei_note','') or '')}</textarea>
      </div>
    </div>
    <div class="ab-card">
    ...
    // 對照規則表（adminB 版、摘要+折疊細節）
    function getCompanyFamily(company) {{
      var prefixes = ['亞太','21','第一','麻吉','和裕','和潤','興達','融易','裕融','分貝'];
      for (var i = 0; i < prefixes.length; i++) {{
        if (company.indexOf(prefixes[i]) === 0) return prefixes[i];
      }}
      // 預付資融 = 手機原融 / 機車原融 / 汽機車二胎 / 手機無卡分期（同一家、4 種專案）
      if (['手機原融','機車原融','汽機車二胎','手機無卡分期'].indexOf(company) >= 0) return '預付資融';
      // 手機分期 是獨立公司、不歸預付資融
      if (['零卡(C)','商品貸','代書','當舖'].indexOf(company) >= 0) return '民間';
      return company;
    }}
    function _planRowHtml(r) {{
      // 單一方案的折疊行
      var icon = r.eligible === true ? '✅' : (r.eligible === 'manual' ? '⚠️' : '❌');
      var bgColor = r.eligible === true ? '#f0fdf4' : (r.eligible === 'manual' ? '#fef3c7' : '#fef2f2');
      var fam = getCompanyFamily(r.company);
      var isCarOrBike = (fam === '21汽車' || fam === '裕融' || fam === '和潤' || r.company === '和潤汽車' || r.company === '和潤重機' || r.company.indexOf('21汽車') === 0);
      var detail = '';
      // 汽車/重機特別註記
      if (isCarOrBike) {{
      ...
      if (r.required_docs && r.required_docs.length) {{
        detail += '<div style="margin-top:6px;font-size:11px;color:#6b7280">📎 ' + r.required_docs.join('、') + '</div>';
      }}
      var html = '<details style="margin-bottom:4px;background:' + bgColor + ';border-radius:5px;padding:6px 10px">';
      html += '<summary style="font-size:13px;font-weight:600;cursor:pointer;list-style:none">';
      html += icon + ' ' + r.company;
      // 同家族不重複顯示金額（汽車/銀行類略過）
      if (!isCarOrBike && fam !== '銀行' && fam !== '民間') {{
        html += ' — ' + r.max_amount + '萬';
      }}
      if (r.eligible === 'manual') html += ' <span style="color:#92400e;font-size:11px;font-weight:400">(需確認)</span>';
      if (r.eligible === false && r.private_blocked) html += ' <span style="color:#dc2626;font-size:11px;font-weight:400">(私設不可送)</span>';
      html += '</summary>';
      html += '<div style="margin-top:6px">' + detail + '</div>';
      html += '</details>';
      return html;
    }}
    function checkEligibilityAdminB() {{
      var cid = '{h(case_id)}';
      var box = document.getElementById('eligibilityBoxAdminB');
      ...
      fetch('/api/check-eligibility?case_id=' + encodeURIComponent(cid))
        ...
        .then(function(d) {{
          ...
          if (!results.length) {{
            box.innerHTML = '<span style="color:#6b7280">目前沒有規則表</span>';
            return;
          }}
          var html = '';
          // 警告 banner（最重要、放最上面）
          if (d.private_status === 'confirmed') {{
            html += '<div style="background:#fef2f2;border:2px solid #dc2626;border-radius:8px;padding:10px 12px;margin-bottom:10px;color:#991b1b;font-weight:700;font-size:13px">🚫 客戶有私設動保、融資不可送、只能看下方「✅ 可送」</div>';
          }} else if (d.unknown_dynbao && d.unknown_dynbao.length) {{
            html += '<div style="background:#fef3c7;border:2px solid #d97706;border-radius:8px;padding:10px 12px;margin-bottom:10px;color:#92400e;font-size:13px">⚠️ 動保來自未見過的公司：<b>' + d.unknown_dynbao.join('、') + '</b><br><span style="font-size:11px">請人工確認是否私設</span></div>';
          }}

          // 分 3 桶
          var canSend = results.filter(function(r){{return r.eligible === true || r.eligible === 'manual';}});
          var cantSend = results.filter(function(r){{return r.eligible === false;}});

          // 不顯示金額的家族（汽車/重機/銀行/代書/當鋪 都看天書/實際業務判斷）
          var noAmountFamilies = ['21汽車','裕融','和潤','銀行','代書','當舖'];
          // 注意：21汽車 是用前綴 indexOf 判斷
          function shouldHideAmount(company, fam) {{
            if (fam === '裕融' || fam === '和潤' || fam === '銀行' || fam === '代書' || fam === '當舖') return true;
            if (company.indexOf('21汽車') === 0) return true;
            if (company === '和潤重機' || company === '和潤汽車') return true;
            return false;
          }}

          // === 摘要：建議送清單（同家族合併成 1 卡）===
          html += '<div style="background:#f0fdf4;border:1px solid #86efac;border-radius:8px;padding:12px;margin-bottom:10px">';
          html += '<div style="font-weight:700;font-size:14px;color:#15803d;margin-bottom:8px">✅ 建議送</div>';
          if (canSend.length === 0) {{
            html += '<div style="color:#6b7280;font-size:13px">無方案可送、請人工再確認</div>';
          }} else {{
            // 按家族聚合
            var sendGrouped = {{}};
            var sendFamOrder = [];
            canSend.forEach(function(r) {{
              var fam = getCompanyFamily(r.company);
              if (!sendGrouped[fam]) {{
                sendGrouped[fam] = [];
                sendFamOrder.push(fam);
              }}
              sendGrouped[fam].push(r);
            }});
            // 家族順序：該家族最高 priority 優先
            sendFamOrder.sort(function(a, b) {{
              var maxA = Math.max.apply(null, sendGrouped[a].map(function(r){{return r.priority;}}));
              var maxB = Math.max.apply(null, sendGrouped[b].map(function(r){{return r.priority;}}));
              return maxB - maxA;
            }});

            html += '<div style="display:grid;grid-template-columns:repeat(auto-fill,minmax(180px,1fr));gap:6px">';
            sendFamOrder.forEach(function(fam) {{
              var plans = sendGrouped[fam];
              // 同家族取最高金額（除非該家族不顯示金額）
              var maxAmt = Math.max.apply(null, plans.map(function(r){{return r.max_amount;}}));
              var hideAmt = shouldHideAmount(plans[0].company, fam);
              // 家族內所有方案是否都需 manual 確認
              var allManual = plans.every(function(r){{return r.eligible === 'manual';}});
              var tag = allManual ? '⚠️' : '✅';
              var color = allManual ? '#92400e' : '#15803d';
              var amtTxt = '';
              if (!hideAmt) amtTxt = '（' + maxAmt + '萬）';
              var planCntTxt = plans.length > 1 ? '［' + plans.length + ' 方案］' : '';
              html += '<div style="background:#fff;border:1px solid #d4d4d4;border-radius:5px;padding:8px 10px;font-size:13px;color:' + color + '">' + tag + ' <b>' + fam + '</b>' + amtTxt + ' ' + planCntTxt + '</div>';
            }});
            html += '</div>';
            html += '<div style="font-size:11px;color:#6b7280;margin-top:8px">⚠️ = 規則 OK 但需人工確認；汽車/重機/銀行/代書/當鋪 金額看天書/權威/鑑價、不顯示固定金額</div>';
          }}
          html += '</div>';

          // === 折疊：依家族顯示細節 ===
          html += '<details style="margin-top:10px"><summary style="cursor:pointer;font-weight:700;font-size:13px;color:#854d0e;padding:6px 0">📋 看完整規則檢查（' + results.length + ' 家、可送+不可送）</summary>';
          ...
          // 家族順序：可送家族 priority 高的優先
          familyOrder.sort(function(a, b) {{
            var maxA = Math.max.apply(null, grouped[a].map(function(r){{return r.priority;}}));
            var maxB = Math.max.apply(null, grouped[b].map(function(r){{return r.priority;}}));
            return maxB - maxA;
          }});

          familyOrder.forEach(function(fam) {{
            var familyPlans = grouped[fam];
            var passNum = familyPlans.filter(function(r){{return r.eligible === true || r.eligible === 'manual';}}).length;
            html += '<div style="margin-top:10px;padding:6px 10px;background:#3a3530;color:#fff;border-radius:5px;font-weight:600;font-size:12px">' + fam + '（' + passNum + '/' + familyPlans.length + ' 可送）</div>';
            familyPlans.forEach(function(r) {{
              html += _planRowHtml(r);
            }});
          }});
        ...
        .catch(function(e) {{ box.innerHTML = '<span style="color:#dc2626">網路錯誤</span>'; }});
    }}
    </script>
    </form>
    </div>
...
async def adminb_save(request: Request):
    ...
    fields = {
        ...
        "adminb_bei_note": form.get("bei_note",""),
...
<div class="card"><div class="sec">身分證發證資料</div><div class="g3">
  ...
  <div><label>發證地</label><select name="idplace" class="ep"><option value="">請選擇</option>{"".join(f'<option {"selected" if v("id_issue_place")==p else ""}>{p}</option>' for p in ["北市","北縣","新北市","桃市","桃縣","中市","中縣","南市","南縣","高市","高縣","基市","竹市","竹縣","苗縣","彰縣","投縣","雲縣","嘉市","嘉縣","屏縣","宜縣","花縣","東縣","澎縣","金門","連江"])}</select></div>
...
<div class="card"><div class="sec">聯絡人</div><div class="g2" style="margin-bottom:12px">
  ...
  <div><label>知情</label><select name="c1know" class="ep"><option {"selected" if v("contact1_known")=="可知情" else ""}>可知情</option><option {"selected" if v("contact1_known")=="保密" else ""}>保密</option></select></div>
</div><div class="g2">
...
<div class="card"><div class="sec">貸款諮詢事項</div><div class="g2">
  ...
  <div><label>勞保狀態</label><select name="elabor" class="ep"><option {"selected" if v("eval_labor_ins")=="公司保" else ""}>公司保</option><option {"selected" if v("eval_labor_ins")=="工會保" else ""}>工會保</option><option {"selected" if v("eval_labor_ins")=="自行投保" else ""}>自行投保</option><option {"selected" if v("eval_labor_ins")=="軍保" else ""}>軍保</option><option {"selected" if v("eval_labor_ins")=="公保" else ""}>公保</option><option {"selected" if v("eval_labor_ins")=="農保" else ""}>農保</option><option {"selected" if v("eval_labor_ins")=="漁保" else ""}>漁保</option><option {"selected" if v("eval_labor_ins")=="無勞保" else ""}>無勞保</option></select></div>
...
// 地址防錯：儲存前檢查詳細地址欄位不可含縣市名
document.querySelector('form[action="/edit-pending"]').addEventListener('submit', function(ev){{
  var CITIES=['台北市','臺北市','新北市','桃園市','台中市','臺中市','台南市','臺南市','高雄市','基隆市','新竹市','新竹縣','苗栗縣','彰化縣','南投縣','雲林縣','嘉義市','嘉義縣','屏東縣','宜蘭縣','花蓮縣','台東縣','臺東縣','澎湖縣','金門縣','連江縣'];
...
def case_edit_get(request: Request, case_id: str = "", saved: str = ""):
    ...
    try:
        ...
        company_status = {}
    # 從 route_plan history 抓每家的核准金額/撥款日
    try:
        _rp_for_cs = parse_route_json(v("route_plan") or "")
    except Exception:
        _rp_for_cs = {"history": []}
    _hist_list = _rp_for_cs.get("history", []) or []

    # 核准明細 list 顯示（從 history 帶出每家金額/撥款日）
    # 若 history 空但 customer 有 approved_amount + current_company → 補一筆
    _ap_rows = []
    for _h in _hist_list:
        if _h.get("amount"):
            _ap_rows.append({
                "company": _h.get("company", ""),
                "amount": _h.get("amount", ""),
                "disbursed": _h.get("disbursed", ""),
            })
    if not _ap_rows and v("approved_amount") and (v("current_company") or v("company")):
        _ap_rows.append({
            "company": v("current_company") or v("company"),
            "amount": v("approved_amount"),
            "disbursed": v("disbursement_date"),
        })
    ap_rows_html = ""
    for _ap in _ap_rows:
        ap_rows_html += (
...
document.querySelector('form[action="/case-edit"]').addEventListener('submit', function() {{
  ...
  document.querySelectorAll('#apBox .ap-row').forEach((r, idx) => {{
    const coEl = r.querySelector('.ap-co');
    const amtEl = r.querySelector('.ap-amt');
    const disbEl = r.querySelector('.ap-disb');
    const co = coEl ? coEl.value.trim() : '';
  ...
  document.getElementById('apVal').value = JSON.stringify(apArr);
  document.getElementById('apAmtVal').value = firstAmt;
  document.getElementById('apDisbVal').value = firstDisb;
}});
...
function checkEligibility() {{
  ...
  fetch('/api/check-eligibility?case_id=' + encodeURIComponent(cid))
    ...
    .then(d => {{
      ...
      // 私設警告 banner
      if (d.private_status === 'confirmed') {{
        html += '<div style="background:#fef2f2;border:2px solid #dc2626;border-radius:8px;padding:12px;margin-bottom:12px;color:#991b1b;font-weight:700">🚫 客戶有確認的私設動保、融資公司皆不可送、僅可送下方標綠的方案</div>';
      }} else if (d.unknown_dynbao && d.unknown_dynbao.length) {{
        html += '<div style="background:#fef3c7;border:2px solid #d97706;border-radius:8px;padding:12px;margin-bottom:12px;color:#92400e">⚠️ 客戶動保來自未見過的公司：' + d.unknown_dynbao.join('、') + '<br>請人工確認是否私設、再決定能否送融資</div>';
      }}
      html += '<div style="font-weight:700;font-size:14px;color:#854d0e;margin-bottom:10px">📋 規則對照結果（依金額大、容易過件排序）</div>';
      results.forEach(r => {{
...
def api_check_eligibility(request: Request, case_id: str = ""):
    ...
    if not row:
        return JSONResponse({"ok": False, "message": "找不到客戶"})
    customer = dict(row)
    eval_result = evaluate_case(customer)
    return JSONResponse({
        "ok": True,
        "results": eval_result["results"],
        "private_status": eval_result["private_status"],
        "unknown_dynbao": eval_result["unknown_dynbao"],
        "house_private": eval_result["house_private"],
    })
...
async def case_edit_post(request: Request):
    ...
    try:
        ...
        cs_json = "{}"
    # 核准明細（從 apVal 來）→ 用清單覆蓋 route_plan history（保留 order/idx）
    ap_hist_json = f.get("approval_history_json") or "[]"
    try:
        ap_hist_list = json.loads(ap_hist_json)
        if not isinstance(ap_hist_list, list):
            ap_hist_list = []
    except Exception:
        ap_hist_list = []
    with db_conn(commit=True) as conn:
        cur = conn.cursor()
        cur.execute("UPDATE customers SET company_status=? WHERE case_id=?", (cs_json, case_id))
        # 核准明細：替換 route_plan["history"]（即使空 list 也清掉、讓使用者刪光生效）
...
async def delete_customer(request: Request):
    """軟刪除單筆客戶（status='DELETED'、從列表/日報消失、DB 還在可還原）
    權限：
    - 從「客戶資料庫」刪除（from=pending）：所有登入者可用
    - 從「日報」刪除（from=report）：只允許 admin
    """
    from fastapi.responses import RedirectResponse
    role = check_auth(request)
    if not role:
        return HTMLResponse("<h3>❌ 未登入</h3>", status_code=401)
    form = await request.form()
    case_id = form.get("case_id", "").strip()
    from_page = form.get("from", "pending").strip()
    if from_page == "report" and role != "admin":
        return HTMLResponse("<h3>❌ 從日報刪除需管理員權限</h3>", status_code=403)
    if not case_id:
    ...
    # 軟刪除：status='DELETED'、保留資料可還原
    now = now_iso()
    cur.execute("UPDATE customers SET status='DELETED', updated_at=? WHERE case_id=?", (now, case_id))
    conn.commit(); conn.close()
    redirect_to = "/report" if from_page == "report" else "/pending-customers?deleted=1"
...
function doSubmit(){
  ...
  // 地址防錯：詳細地址欄位不可含「縣/市/區/鄉/鎮/鄉鎮」— 這些要拆到左邊欄位
  var CITIES=['台北市','臺北市','新北市','桃園市','台中市','臺中市','台南市','臺南市','高雄市','基隆市','新竹市','新竹縣','苗栗縣','彰化縣','南投縣','雲林縣','嘉義市','嘉義縣','屏東縣','宜蘭縣','花蓮縣','台東縣','臺東縣','澎湖縣','金門縣','連江縣'];
  function addrHasCity(s){if(!s)return '';for(var i=0;i<CITIES.length;i++){if(s.indexOf(CITIES[i])===0)return CITIES[i];}return '';}
  var rdist=qq('[name="rdist"]').trim();
  var ldist=qq('[name="ldist"]').trim();
  var cdist=qq('[name="cdist"]').trim();
  var raHit=addrHasCity(ra); if(raHit)e.push('戶籍詳細地址不可含縣市名「'+raHit+'」，請拆到左邊縣市欄位');
  var laHit=addrHasCity(la); if(!sa&&laHit)e.push('住家詳細地址不可含縣市名「'+laHit+'」，請拆到左邊縣市欄位');
  var caHit=addrHasCity(ca); if(caHit)e.push('公司詳細地址不可含縣市名「'+caHit+'」，請拆到左邊縣市欄位');
  // 詳細地址若含「區/鄉/鎮」關鍵字、但「區」欄位空白 → 提示使用者拆開
  function hasDistWord(s){return /[區鄉鎮市]/.test(s||'');}
...
<a href="/new-customer" class="btn2">繼續新增其他客戶</a>
    ...
    extra = dict(
        ...
        eval_sent_3m_detail=f.get("esent_detail",""),
...
<tr><th>電信業者</th><td>{v("carrier")}</td><th>Email</th><td>{v("email")}</td></tr>
<tr><th>LINE ID</th><td>{v("line_id")}</td><th>客戶FB</th><td>{v("fb")}</td></tr>
    ...
    def _build_txt_content(template_path: str, r: dict) -> str:
        ...
        def find_value(label_text):
            ...
            if contact_state["idx"] == 1:
                ...
                if "姓名" in t: return True, v("contact1_name")  # 含「1.姓名」
            ...
            if contact_state["idx"] == 2:
                ...
                if "姓名" in t: return True, v("contact2_name")  # 含「2.姓名」
...
function exportPDF(){
  ...
  trows+=s3('職業資料',
    ...
    r1('公司地址',caddr));
  function rContact(label,name,rel,tel,known){
    var subTbl='<table style="width:100%;border-collapse:collapse;margin:0">'+
      '<tr>'+
      '<th style="background:#f0ebe4;width:50px;border:0;border-right:1px solid #bbb;padding:5px 8px;font-weight:600;color:#3a3020;font-size:12px">姓名</th>'+
      '<td style="border:0;border-right:1px solid #bbb;padding:5px 8px;background:#fff">'+(name||'-')+'</td>'+
      '<th style="background:#f0ebe4;width:50px;border:0;border-right:1px solid #bbb;padding:5px 8px;font-weight:600;color:#3a3020;font-size:12px">關係</th>'+
      '<td style="border:0;border-right:1px solid #bbb;padding:5px 8px;background:#fff">'+(rel||'-')+'</td>'+
      '<th style="background:#f0ebe4;width:50px;border:0;border-right:1px solid #bbb;padding:5px 8px;font-weight:600;color:#3a3020;font-size:12px">電話</th>'+
      '<td style="border:0;border-right:1px solid #bbb;padding:5px 8px;background:#fff">'+(tel||'-')+'</td>'+
      '<th style="background:#f0ebe4;width:50px;border:0;border-right:1px solid #bbb;padding:5px 8px;font-weight:600;color:#3a3020;font-size:12px">知情</th>'+
      '<td style="border:0;padding:5px 8px;background:#fff">'+(known||'-')+'</td>'+
      '</tr></table>';
    return '<tr><th>'+label+'</th><td colspan="3" style="padding:0 !important">'+subTbl+'</td></tr>';
  }
  trows+=s3('聯絡人資料',
    rContact('聯絡人1',gv('c1name'),gv('c1rel'),gv('c1tel'),gv('c1know'))+
    rContact('聯絡人2',gv('c2name'),gv('c2rel'),gv('c2tel'),gv('c2know')));
  trows+=s3('貸款諮詢事項',
