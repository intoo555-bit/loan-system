    ...
    for disb_date, companies in parsed.items():
        for company, names in companies.items():
            for name in names:
                ...
                # 推送到對應行政群（每個業務群各自推，不重複）
                admin_gid = get_admin_group_for_sales(target["source_group_id"])
                if admin_gid:
                    push_key = admin_gid + "_" + disb_date + "_" + company
                    if push_key not in notified_admin_groups:
                        name_list = chr(10).join(n for n in companies[company])
                        push_msg = disb_date + " " + company + " 撥款名單" + chr(10) + name_list
                        push_text(admin_gid, push_msg)
                        notified_admin_groups.add(push_key)
