...
def new_customer_page(request: Request):
    ...
    # 在表單前面插入智能填入區塊
    smart_fill_html = '''<div class="card" style="background:#fffbeb;border-color:#fde68a">
      <div class="section-title" style="color:#92400e;cursor:pointer" onclick="document.getElementById('smartbox').style.display=document.getElementById('smartbox').style.display==='none'?'block':'none'">📋 智能填入（點擊展開）</div>
      <div id="smartbox" style="display:none">
        <textarea id="smarttext" style="width:100%;min-height:120px;padding:8px;border:1px solid #c8bfb5;border-radius:6px;font-size:13px;font-family:inherit;margin-bottom:8px" placeholder="把業務給的客戶資料文字貼在這裡..."></textarea>
        <button type="button" onclick="smartFill()" style="background:#92400e;color:#fff;border:none;padding:8px 20px;border-radius:8px;font-size:14px;font-weight:600;cursor:pointer">智能填入</button>
        <span id="smartresult" style="margin-left:10px;font-size:13px;color:#166534"></span>
      </div>
    </div>'''
    HTML_PAGE = HTML_PAGE.replace('<form id="cf"', smart_fill_html + '\n  <form id="cf"')
