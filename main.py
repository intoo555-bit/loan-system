...
# 主要業務邏輯
# =========================
def handle_bc_case_block(block_text: str, source_group_id: str, reply_token: str, has_ai_in_message: bool = False):
    action_words = [
...
async def callback(request: Request):
    ...
    for event in body.get("events", []):
        ...
        if group_id in [B_GROUP_ID, C_GROUP_ID]:
            ...
            clean_text = re.sub(r"(@ai助理|#ai助理|@ai|#ai)", "", raw_text, flags=re.IGNORECASE).strip()
            ...
            for idx, block in enumerate(blocks, start=1):
                result = handle_bc_case_block(block, group_id, reply_token, has_ai_in_message=bc_has_ai_trigger)
