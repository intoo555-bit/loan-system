    ...
    <div class="ab-card">
      ...
      <div class="ab-block" style="background:#e0f2fe;">
        ...
        <div class="ab-g3" style="margin-bottom:10px;">
          <div><div class="ab-lbl">資金用途</div><select name="at_fund" class="ab-sel"><option value="II-3購買3C產品">II-3購買3C產品</option><option value="II-1購買交通工具">II-1購買交通工具</option><option value="II-2購買手機">II-2購買手機</option><option value="V-1生活周轉金">V-1生活周轉金</option></select></div>
...
def new_customer_page(request: Request):
    ...
    group_opts = "".join(f'<option value="{h(g["group_id"])}">{h(g["group_name"])}</option>' for g in groups)
...
function doSubmit(){
  ...
  b2.innerHTML='<div style="color:#854d0e;font-weight:700;margin-bottom:8px;">人工確認事項：</div>'+w.map(function(x){return '<div style="color:#854d0e;padding:2px 0;">'+x+'</div>';}).join('')+'<div style="margin-top:10px;"><button onclick="document.forms[0].submit()" style="background:#6a5e4e;color:#fff;border:none;padding:8px 18px;border-radius:6px;cursor:pointer;">確認無誤，送出</button></div>';
...
"+pdf_js+"
"""
    HTML_PAGE = HTML_PAGE.replace('addV();addD();', inject_js+'addV();addD();')
    return HTML_PAGE
